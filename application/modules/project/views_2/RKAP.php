...
<div class="page-content">
    ...
    <!-- sql tahun -->
    <?php $dataTahun = $this->db->query("SELECT pekerjaan_tahun as tahun FROM dec.dec_pekerjaan WHERE pekerjaan_tahun IS NOT NULL GROUP BY pekerjaan_tahun ORDER BY pekerjaan_tahun ASC")->result_array(); ?>

    <?php
    $sesi = $this->session->userdata();
    $admin_sistem = '';
    $admin_sistem = $this->db->query("SELECT * FROM global.global_admin WHERE admin_nik = '" . $sesi['pegawai_nik'] . "'")->row_array();
    ...
    <script type="text/javascript">
        ...
        $(function() {
                    ...
                    },
                    {
                        render: function(data, type, full, meta) {
                            ...
                            if (full.milik == 'y' && full.pekerjaan_nomor == null) {
                                nomor_isi = '';
                            } else if (full.milik == 'y' && <?= $data_session['pegawai_nik'] ?> != <?= $admin_sistemnya ?>) {
            ...
            var table_ifa = $('#table_ifa').DataTable({
                ...
                "scrollX": true,
                "ajax": {
                    "url": "<?= base_url('project/') ?>pekerjaan_usulan/getPekerjaanUsulan?klasifikasi_pekerjaan_id=1&pekerjaan_status=8,9,10",
                    "dataSrc": ""
                },
                "columns": [{
                        render: function(data, type, full, meta) {
            ...
            $('#table_ifc').DataTable({
                ...
                // "scrollX": true,
                "ajax": {
                    "url": "<?= base_url() ?>project/pekerjaan_usulan/getPekerjaanUsulan?klasifikasi_pekerjaan_id=1&pekerjaan_status=11,12,13",
                    "dataSrc": ""
                },
                "columns": [{
                        render: function(data, type, full, meta) {
