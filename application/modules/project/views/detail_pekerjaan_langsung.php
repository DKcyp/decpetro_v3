...
$sql_bagian = ($this->input->get('id_user') != '') ? $this->db->query("SELECT * FROM global.global_bagian a LEFT JOIN global.global_bagian_detail b ON a.bagian_id = b.id_bagian LEFT JOIN global.global_pegawai c ON c.pegawai_nik = b.id_pegawai WHERE b.id_pegawai = '" . $this->input->get('id_user') . "'") : $this->db->query("SELECT * FROM global.global_bagian a LEFT JOIN global.global_bagian_detail b ON a.bagian_id = b.id_bagian LEFT JOIN global.global_pegawai c ON c.pegawai_nik = b.id_pegawai WHERE b.id_pegawai = '" . $data_session['pegawai_nik'] . "'");
$data_bagian = $sql_bagian->row_array();

$sql_disposisi = $this->db->query("SELECT id_user,pegawai_nama,pegawai_nik,pekerjaan_disposisi_status FROM dec.dec_pekerjaan_disposisi a LEFT JOIN global.global_pegawai b ON a.id_user = b.pegawai_nik WHERE a.is_aktif = 'y' AND a.id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' AND CAST(pekerjaan_disposisi_status as INT) <= '13' GROUP BY id_user,pegawai_nama,pegawai_nik,pekerjaan_disposisi_status,id_penanggung_jawab ORDER BY CAST(pekerjaan_disposisi_status AS INT) ASC,id_penanggung_jawab DESC ");
$data_disposisi = $sql_disposisi->result_array();

$sql_disposisi_user = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE id_user = '" . $this->input->get('id_user') . "' AND id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' AND pekerjaan_disposisi_status = '" . $this->input->get('status') . "'");
$data_disposisi_user = $sql_disposisi_user->row_array();

$sql_disposisi_cc = $this->db->query("SELECT id_user,pegawai_nama,pegawai_nik,pekerjaan_disposisi_status FROM dec.dec_pekerjaan_disposisi a LEFT JOIN global.global_pegawai b ON a.id_user = b.pegawai_nik WHERE a.is_aktif = 'y' AND a.id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' AND is_cc != '' AND pekerjaan_disposisi_status = '8' GROUP BY id_user,pegawai_nama,pegawai_nik,pekerjaan_disposisi_status ORDER BY CAST(pekerjaan_disposisi_status AS INT) ASC");
$data_disposisi_cc = $sql_disposisi_cc->result_array();

$data_selesai = '';

if ($this->input->get('status') != '-') {
  $data_selesai = $this->db->query("SELECT COUNT(*) AS total FROM dec.dec_pekerjaan WHERE pekerjaan_id ='" . $this->input->get('pekerjaan_id') . "' AND CAST(pekerjaan_status as INT) >='14'")->row_array();
}
?>
...
<div class="page-content">
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-12">
        <a href="<?= base_url('project/pekerjaan_langsung') ?>" class="btn btn-success"><u><i class="fa fa-arrow-left"></i> Kembali</u></a>
      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-body">
            ...
            <label>User</label>
            <select class="form-control" id="filter_user" name="filter_user" onchange="fun_refresh('<?= $this->input->get('pekerjaan_id') ?>', this.value)">
              <option value="">- Pilih User -</option>
              <?php foreach ($data_disposisi as $value) : ?>
                <?php
                unset($tree);
                for ($j = 0; $j < $value['pekerjaan_disposisi_status']; $j++) $tree .= ". ";
                ?>
                <option value="<?= $value['id_user'] . '~' . $value['pekerjaan_disposisi_status'] ?>" <?php if ($this->input->get('id_user') == $value['id_user'] && $this->input->get('status') == $value['pekerjaan_disposisi_status']) echo 'selected' ?>><?= $tree . $value['pegawai_nama'] ?></option>
              <?php endforeach ?>
            </select>
            <?php if ($data_selesai) : ?>
              <?php if ($data_selesai['total'] > 0) : ?>
                <label>CC Selesai</label>
                <select class="form-control" id="filter_user" name="filter_user" onchange="fun_refresh_cc('<?= $this->input->get('pekerjaan_id') ?>', this.value)">
                  <option value="">- Pilih User -</option>
                  <?php foreach ($data_disposisi_cc as $value) : ?>
                    <?php
                    unset($tree);
                    for ($j = 0; $j < $value['pekerjaan_disposisi_status']; $j++) $tree .= ". ";
                    ?>
                    <option value="<?= $value['id_user'] . '~' . $value['pekerjaan_disposisi_status'] ?>" <?php if ($this->input->get('id_user_cc') == $value['id_user'] && $this->input->get('status') == $value['pekerjaan_disposisi_status']) echo 'selected' ?>><?= $tree . $value['pegawai_nama'] ?></option>
                  <?php endforeach ?>
                </select>
              <?php endif; ?>
            <?php endif; ?>
          ...
          <div class="card-body">
            <div class="d-flex">
              <div class="flex-grow-1 overflow-hidden">
                ...
                <input type="text" name="is_rkap" id="is_rkap" value="<?= $this->input->get('rkap'); ?>" hidden>
            ...
            <div class="d-flex">
              ...
              <div class="d-flex">
                <div class="flex-grow-1 overflow-hidden">
                  ...
                  <p class="text-muted"><?= (!empty($data_disposisi_user['pekerjaan_disposisi_catatan'])) ? ($data_disposisi_user['pekerjaan_disposisi_catatan']) : '-' ?></p>
                </div>
              </div>
              <?php if ($data_disposisi_user && $data_disposisi_user['pekerjaan_disposisi_status'] == '4') : ?>
                <div class="d-flex">
                  <div class="flex-grow-1 overflow-hidden">
                    <h5 class="text-truncate font-size-15">Prioritas Pekerjaan</h5>
                    <p class="text-muted">
                      <?php
                      switch ($data_disposisi_user['pekerjaan_disposisi_prioritas']) {
                        case '1':
                        echo 'Normal';
                        break;
                        case '2';
              ...
              <?php if ($this->input->get('aksi') == 'berjalan') : ?>
                <div class="d-flex">
                  <div class="flex-grow-1 overflow-hidden">
                    ...
                    <p class="text-muted">
                      <?php
                      switch ($data_disposisi_user['pekerjaan_disposisi_kategori']) {
                        case '1':
                        echo 'Mudah';
                        break;
                        case '2';
              ...
              <!-- <div class="d-flex">
                  <div class="flex-grow-1 overflow-hidden">
                    <h5 class="text-truncate font-size-15"><i class="bx bx-calendar me-1 text-primary"></i> Tanggal Akhir Pekerjaan</h5>
                    <p class="text-muted">
                      <?= (!empty($pekerjaan)) ? date("d-m-Y", strtotime($pekerjaan['pekerjaan_waktu_akhir'])) : '-' ?>
                    </p>
                  </div>
                </div> -->
              <?php endif ?>
              ...
              <div class="row">
                ...
                <?php if ($this->input->get('aksi') == 'usulan') : ?>
                  ...
                  <div id="btn_approve" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-primary col-10" onclick="fun_approve('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Approved?')">Approve</button>
                    </div>
                  </div>
                  <div id="btn_reviewed" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-primary col-10" onclick="fun_review('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Reviewed?')">Reviewed</button>
                    </div>
                  </div>
                  <div id="btn_disposisi_vp" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-primary col-10" onclick="fun_disposisi_vp('<?= $this->input->get('pekerjaan_id') ?>')">Disposisi ke AVP</button>
                    </div>
                  </div>
                  <div id="btn_disposisi_avp" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-primary col-10" onclick="fun_disposisi_avp('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>');fun_disposisi_avp_check_tj('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')">Reviewed AVP</button>
                    </div>
                  </div>
                  ...
                  <div id="btn_reject" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-danger col-10" onclick="fun_reject('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')">Reject</button>
                    </div>
                  </div>
                  <div id="btn_reject_avp" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-danger col-10" onclick="fun_reject_avp('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')">Reject AVP</button>
                    </div>
                  </div>
                  <!-- Usulan -->
                ...
                <?php elseif ($this->input->get('aksi') == 'ifc') : ?>
                  ...
                  <div class="col-sm-4 col-md-3" id="btn_upload_ifc_hps" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-success col-10" onclick="fun_upload_ifc_hps('<?= $this->input->get('pekerjaan_id') ?>','<?= $this->input->get('status') ?>')">Upload Dok Internal</button>
                    </div>
                  </div>
                  <div class="col-sm-4 col-md-3" id="btn_upload_ifc" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-success col-10" onclick="fun_upload_ifc('<?= $this->input->get('pekerjaan_id') ?>','<?= $this->input->get('status') ?>')">Upload Dokumen</button>
                    </div>
                  </div>
                  ...
                  <div id="btn_reject_staf" class="col-sm-4 col-md-3" style="display: none;">
                    <div class="mt-4">
                      <button type="button" class="btn btn-danger col-10" onclick="fun_reject_staf_ifc('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')">Reject Staf</button>
                    </div>
                  </div>
                  <!-- IFC -->
                <?php endif ?>
        ...
        <?php elseif ($this->input->get('aksi') == 'berjalan') : ?>
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
                <!-- Tombol -->
                <!-- <button type="button" class="btn btn-primary col-1 float-end" id="btn_send_avp" onclick="fun_approve_berjalan('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Reviewed?')" style="display: none;">Send VP</button> -->
                ...
                <button type="button" class="btn btn-success col-2 float-end" onclick="funcModalSendVPKoor(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_send_avp" style="display:none">
                  Send VP
                </button>
                <button type="button" class="btn btn-info col-2 float-end" onclick="funcModalSendVP(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_send_avp_koor" style="display:none">
                  Send AVP Koor
                </button>
                <button type="button" class="btn btn-success col-2 float-end" onclick="funcModalApproveVP(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_approve_vp" style="display:none">
                  Send User
                </button>
                <button type="button" class="btn btn-danger col-1 float-end" id="btn_revisi" onclick="fun_reject_berjalan_ifa('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')" style="display: none;">Revisi</button>
                <!-- Tombol -->
              </div>
            </div>
          </div>
        ...
        <div class="col-lg-12">
          <div class="card"> -->
            <div id="div_doc_ifa" style="display:none">
              <div class="card-body">
                ...
                <table class="table table-bordered table-striped" id="table_dokumen" width="100%">
                  <thead>
                    <tr>
                      <th>Pilih</th>
                      <th>No</th>
              ...
              <div class="card"> -->
                <div id="div_doc_ifa_hps" style="display:none">
                  <div class="card-body">
                    ...
                    <table class="table table-striped align-middle mb-0" id="table_dokumen_hps" width="100%">
                      <thead>
                        <tr>
                          <th>Pilih</th>
                          <th>No</th>
            ...
            <?php elseif ($this->input->get('aksi') == 'ifa') : ?>
              ...
              <div class="col-lg-12">
                <div class="card">
                  <div class="card-body">
                    <!-- Tombol -->
                    <button type="button" class="btn btn-success col-1 float-end" onclick="funcModalSendVPIFC(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_send_avp_ifc" style="display:none">
                      Send VP
                    </button>

                    <button type="button" class="btn btn-danger col-1 float-end" id="btn_revisi" onclick="fun_reject_berjalan('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')" style="display: none;">Revisi</button>

                    <button type="button" class="btn btn-success col-2 float-end" id="btn_approve_ifa" onclick="fun_approve_ifa('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Approved?')" style="display: none;">Approve IFA</button>

                    <button type="button" class="btn btn-success col-2 float-end" id="btn_approve_ifa_avp" onclick="fun_approve_ifa_avp('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Approved?')" style="display: none;">
                      Approve IFA AVP
                    </button>

                    <button type="button" class="btn btn-success col-2 float-end" id="btn_approve_ifa_vp" onclick="fun_approve_ifa_vp('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Approved?')" style="display: none;">
                      Approve IFA VP
                    </button>

                    <button type="button" class="btn btn-danger col-1 float-end" id="btn_revisi_ifa" onclick="fun_reject_ifa('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')" style="display: none;">Revisi IFA</button>
                    <button type="button" class="btn btn-danger col-1 float-end" id="btn_revisi" onclick="fun_reject_berjalan_ifa('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')" style="display: none;">Revisi</button>
                    <!-- Tombol -->
                  </div>
                </div>
                ...
                <div class="card">
                  <div class="card-body">
                    ...
                    <ul class="nav nav-tabs">
                      ...
                      <li class="nav-item">
                        <a class="nav-link" href="javascript:;" onclick="div_doc_ifc_hps()" id="link_div_doc_ifc_hps" style="display: none;">IFC (HPS)</a>
                      </li>
                    </ul>
                  </div>
          ...
          <div class="col-lg-12">
            <div class="card"> -->
              <div id="div_doc_ifa_hps" style="display:none">
                <div class="card-body">
                  <h4 class="card-title mb-4">Dokumen IFA Dok Internal</h4>
                  <h4>
                    <button class="btn btn-success" id="pdf_dokumen" name="pdf_dokumen" onClick="pdf_dokumen()">PDF</button>
                  </h4>
        ...
        <?php elseif ($this->input->get('aksi') == 'ifc') : ?>
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
                ...
                <button type="button" class="btn btn-info col-2 float-end" onclick="funcModalSendAVPIFC(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_send_avp_ifc_koor" style="display:none">
                  Send AVP Koor
                </button>
                <button type="button" class="btn btn-success col-2 float-end" onclick="funcModalApproveVP(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_approve_vp" style="display:none">
                  Approve VP
                </button>
                <button type="button" class="btn btn-danger col-1 float-end" id="btn_revisi" onclick="fun_reject_berjalan_ifc('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>')" style="display: none;">Revisi</button>
                <!-- Tombol -->
              </div>
            </div>
          </div>
          ...
          <div class="col-lg-12">
            <div class="card"> -->
              <div id="div_doc_ifa_hps" style="display:none">
                <div class="card-body">
                  <h4 class="card-title mb-4">Dokumen IFA HPS</h4>
                  <h4>
                    <button class="btn btn-success" id="pdf_dokumen" name="pdf_dokumen" onClick="pdf_dokumen()">PDF</button>
                  </h4>
                  <table class="table table-bordered table-striped" id="table_dokumen_ifa_hps" width="100%">
              ...
              <div id="div_doc_ifc_hps" style="display:none">
                <div class="card-body">
                  <h4 class="card-title mb-4">Dokumen IFC HPS</h4>
                  <h4>
                    <button class="btn btn-success" id="pdf_dokumen" name="pdf_dokumen" onClick="pdf_dokumen()">PDF</button>
                  </h4>
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
                ...
                <!-- <button type="button" class="btn btn-primary col-1 float-end" id="btn_send_avp" onclick="fun_approve_berjalan('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Reviewed?')" style="display: none;">Send VP</button> -->
                <!-- <button type="button" class="btn btn-success col-1 float-end" data-bs-toggle="modal" data-target="#modal_send_vp" style="display:none" id="btn_send_avp">Send VP</button> -->
                <button type="button" class="btn btn-success col-2 float-end" onclick="funcModalSendVP(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_send_avp" style="display:none">
                  Send VP
                </button>
                <button type="button" class="btn btn-info col-2 float-end" onclick="funcModalSendVPKoor(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_send_avp_koor" style="display:none">
                  Send VP Koor
                </button>
                <button type="button" class="btn btn-success col-2 float-end" onclick="funcModalApproveVP(<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>)" id="btn_approve_vp" style="display:none">
                  Approve VP
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form_modal_vp">
              <input type="text" name="id_pekerjaan_vp" id="id_pekerjaan_vp" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form_modal_avp">
              <input type="text" name="id_pekerjaan_avp" id="id_pekerjaan_avp" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none;">
              ...
              <div class="card-body row">
                ...
                <div class="form-group row col-md-12" id="div_pekerjaan_waktu_akhir_avp" style="display:none">
                  <label class="col-md-4">Akhir Waktu Pekerjaan</label>
                  <input type="date" name="pekerjaan_waktu_akhir_avp" id="pekerjaan_waktu_akhir_avp" class="form-control col-md-8" value="<?= (!empty($isi_pekerjaan)) ? date("Y-m-d", strtotime($isi_pekerjaan['pekerjaan_waktu_akhir'])) : date('Y-m-d') ?>">
                </div>
                ...
                <div class="form-group row col-md-12" id="div_pekerjaan_catatan_disposisi">
                  <label class="col-md-4">Catatan Pekerjaan</label>
                  <textarea name="pekerjaan_catatan_disposisi" id="pekerjaan_catatan_disposisi" class="form-control col-md-8" placeholder="Catatan Pekerjaan"></textarea>
                </div>
              </div>
              <div class="modal-footer justify-content-between">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form_modal_progress">
              <input type="text" name="id_pekerjaan_progress" id="id_pekerjaan_progress" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form_modal_ganti_perencana">
              <input type="text" name="id_pekerjaan_ganti_perencana" id="id_pekerjaan_ganti_perencana" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
              <input type="text" name="pekerjaan_status_ganti_perencana" id="pekerjaan_status_ganti_perencana" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('status')) ?>" hidden>
              <div class="card-body row">
                <div class="form-group row col-md-12">
          ...
          <div class="modal-body">
            <form id="form_upload_dokumen">
              ...
              <div class="modal-footer justify-content-between">
                ...
                <?php if ($this->input->get('aksi') == 'ifc') : ?>
            ...
            <form id="form_upload_dokumen_hps">
              ...
              <div class="modal-footer justify-content-between">
                ...
                <?php else : ?>
                  ...
                  <!-- <input type="submit" class="btn btn-success pull-right" id="simpan_dokumen_hps" value="Send IFA"> -->
                <?php endif; ?>
          ...
          <div class="modal-body">
            <form id="form_upload_dokumen_ifc">
              ...
              <div class="modal-footer justify-content-between">
                ...
                <?php else : ?>
                  <input type="submit" class="btn btn-success pull-right" id="simpan_dokumen_ifc" value="Send IFA">
                <?php endif; ?>
          ...
          <div class="modal-body">
            <form id="form_upload_dokumen_ifc_hps">
              ...
              <div class="modal-footer justify-content-between">
                ...
                <button type="button" id="close_dokumen_ifc_hps" class="btn btn-primary" onclick="fun_draft_dokumen_ifc_hps()">Draft</button>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form_modal_aksi_ifa_cc">
          <input type="text" name="pekerjaan_dokumen_status_ifa_cc" id="pekerjaan_dokumen_status_ifa_cc" style="display:none">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form_modal_send_vp">
          <input type="text" name="id_pekerjaan_send_vp" id="id_pekerjaan_send_vp" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
          <div class="card-body row">
            <div class="form-group row col-md-12">
              <label class="col-md-4">CC</label>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form_modal_send_vp_koor">
          <input type="text" name="id_pekerjaan_send_vp_koor" id="id_pekerjaan_send_vp_koor" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
          <div class="card-body row">
            <div class="form-group row col-md-12">
              <label class="col-md-4">CC</label>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form_modal_send_avp_ifc">
          <input type="text" name="id_pekerjaan_send_avp_ifc" id="id_pekerjaan_send_avp_ifc" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
          <div class="card-body row">
            <div class="form-group row col-md-12">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form_modal_send_vp_ifc">
          <input type="text" name="id_pekerjaan_send_vp_ifc" id="id_pekerjaan_send_vp_ifc" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
          <div class="card-body row">
            <div class="form-group row col-md-12">
              <label class="col-md-4">CC</label>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form_modal_approve_vp">
          <input type="text" name="id_pekerjaan_approve_vp" id="id_pekerjaan_approve_vp" value="<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>" style="display: none">
          <div class="card-body row">
            <div class="form-group row col-md-12">
              <label class="col-md-4">CC</label>
...
<script type="text/javascript">
  setTimeout(function() {
    $.getJSON('<?= base_url('project/pekerjaan_langsung/getPekerjaan') ?>', {
  ...
  $(function() {
    $.getJSON('<?= base_url('project/pekerjaan_langsung/getPekerjaan') ?>', {
      ...
      if ('<?= $this->input->get('aksi') ?>' == 'usulan') {
        /* Usualan */
        if (json.pekerjaan_disposisi_status == '1') {
          /* Reviewed AVP Customer */
          $('#btn_reviewed').show()
          $('#btn_approve').hide();
          $('#btn_disposisi_vp').hide();
          $('#btn_disposisi_avp').hide();
          $('#btn_reject').show()
          $('#btn_reject_avp').hide();
          /* Reviewed AVP Customer */
        } else if (json.pekerjaan_disposisi_status == '2') {
          /* Approve VP Customer */
          $('#btn_reviewed').hide();
          $('#btn_approve').show()
          $('#btn_disposisi_vp').hide();
          ...
          $('#btn_reject_avp').hide();
          /* Approve VP Customer */
        } else if (json.pekerjaan_disposisi_status == '3') {
          /* Approved VP Cangun */
          $('#btn_reviewed').hide();
          $('#btn_approve').hide();
          $('#btn_disposisi_vp').show()
          $('#btn_disposisi_avp').hide();
          $('#btn_reject').show()
          $('#btn_reject_avp').hide();
          /* Approved VP Cangun */
        } else if (json.pekerjaan_disposisi_status == '4' && json.id_penanggung_jawab == 'y') {
          /* Reviewed AVP Cangun Koor*/
          $('#btn_reviewed').hide();
          $('#btn_approve').hide();
          $('#btn_disposisi_vp').hide();
          $('#btn_disposisi_avp').show()
          $('#btn_reject').hide();
          $('#btn_reject_avp').show()
          $('#btn_ganti_koor').show()
          /* Reviewed VP Cangun Koor*/
        } else if (json.pekerjaan_disposisi_status == '4') {
          /* Reviewed AVP Cangun */
          $('#btn_reviewed').hide();
          $('#btn_approve').hide();
          $('#btn_disposisi_vp').hide();
        ...
        /* Berjalan */
        if (json.pekerjaan_disposisi_status == '5' && json.id_penanggung_jawab == 'y') {
          /* Perencana Koor */
          $('#btn_upload').show()
          $('#btn_upload_hps').show()
          $('#btn_progress').show()
          ...
          $('#btn_ganti_koor').hide();
          /* Perencana Koor */
        } else if (json.pekerjaan_disposisi_status == '5') {
          /* Perencana */
          $('#btn_upload').show()
          $('#btn_upload_hps').show()
          $('#btn_progress').show()
          ...
          $('#btn_ganti_koor').hide();
          /* Perencana */
        } else if (json.pekerjaan_disposisi_status == '6' && json.id_penanggung_jawab == 'y') {
          /* Reviewed AVP Cangun Koor*/
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          ...
          /* Reviewed AVP Cangun Koor*/
        } else if (json.pekerjaan_disposisi_status == '6' && json.id_penanggung_jawab == 'n') {
          /* Reviewed AVP Cangun */
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          $('#btn_send_ifa').hide();
          $('#btn_send_avp').hide();
          $('#btn_send_avp_koor').show()
          $('#btn_approve_vp').hide();
          $('#btn_revisi').show()
          $('#btn_reject_staf').hide();
          $('#btn_ganti_perencana').hide();
          /* Reviewed AVP Cangun */
        } else if (json.pekerjaan_disposisi_status == '7') {
          /* Approve VP Cangun */
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          ...
          $('#btn_reject_staf').hide();
          $('#btn_ganti_perencana').hide();
        } else if (json.pekerjaan_disposisi_status == '8' && json.is_pic == 'y') {
          /* Approve PIC */
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          ...
          $('#btn_ganti_perencana').hide();
          /* Approve PIC */
        } else if (json.pekerjaan_disposisi_status == '8') {
          /* CC */
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          ...
          $('#btn_ganti_perencana').hide();
          /* CC */
        } else if (json.pekerjaan_disposisi_status == '9') {
          /*Approve IFA AVP*/
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          ...
          $('#btn_ganti_perencana').hide();
          /*Approve IFA AVP*/
        } else if (json.pekerjaan_disposisi_status == '10') {
          /*Approve IFA VP*/
          $('#btn_upload').hide();
          $('#btn_upload_hps').hide();
          $('#btn_progress').hide();
          ...
          $('#btn_ganti_perencana').hide();
          /*Approve IFA VP*/
        } else if (json.pekerjaan_disposisi_status == '11') {
          /* Perencana IFC */
          $('#btn_upload_ifc').show()
          $('#btn_upload_ifc_hps').show()
          $('#btn_progress').hide();
          $('#btn_send_ifa').hide();
          $('#btn_send_avp').hide();
          $('#btn_send_avp_koor').hide();
          ...
          $('#btn_ganti_perencana').hide();
          /* Perencana IFC */
        } else if (json.pekerjaan_disposisi_status == '12' && json.id_penanggung_jawab == 'y') {
          /* Reviewed AVP Cangun Koor */
          $('#btn_upload').hide();
          $('#btn_progress').hide();
          $('#btn_send_ifa').hide();
          $('#btn_send_avp_ifc').show()
          $('#btn_send_avp_ifc_koor').hide();
          $('#btn_approve_vp').hide();
          $('#btn_revisi').show()
          $('#btn_reject_staf').hide();
          $('#btn_ganti_perencana').hide();
          /* Reviewed AVP Cangun Koor */
        } else if (json.pekerjaan_disposisi_status == '12' && json.id_penanggung_jawab == 'n') {
          /* Reviewed AVP Cangun */
          $('#btn_upload').hide();
          $('#btn_progress').hide();
          $('#btn_send_ifa').hide();
          $('#btn_send_avp_ifc').hide();
          $('#btn_send_avp_ifc_koor').show()
          $('#btn_approve_vp').hide();
          $('#btn_revisi').show()
          $('#btn_reject_staf').hide();
          $('#btn_ganti_perencana').hide();
          /* Reviewed AVP Cangun */
        } else if (json.pekerjaan_disposisi_status == '13') {
          /* Approve VP Cangun */
          $('#btn_upload').hide();
          $('#btn_progress').hide();
          $('#btn_send_ifa').hide();
      ...
      $('#pekerjaan_status').val(json.pekerjaan_disposisi_status);
    });
    /*load tombol*/
...
$('#table_dokumen_usulan').DataTable({
  ...
  {
    render: function(data, type, full, meta) {
      return full.pekerjaan_dokumen_nama;
    }
  },
  {
    "render": function(data, type, full, meta) {
...
$('#table_dokumen').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenBerjalan?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&is_hps=n&id_user=<?= $this->input->get('id_user') ?>",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
  ...
  },
  {
    "render": function(data, type, full, meta) {
      var aksi = '';
      if (full.id_bagian == $('#session_bagian').val() && full.avp == 'y' && full.is_proses != 'y' && full.vp != 'y' && full.pekerjaan_dokumen_status != '0') {
      ...
      } else if (full.is_proses == 'y' && full.vp == 'y' && full.pekerjaan_dokumen_status != '0' && full.pekerjaan_dokumen_status == '3') {
        aksi = '<center><a href="javascript:;" id="' + full.pekerjaan_dokumen_id + '" title="Aksi" onclick="fun_aksi(this.id)"><i class="fa fa-share"></i></a></center>';
...
$('#table_dokumen_hps').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenBerjalan?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&id_user=<?= $this->input->get('id_user') ?>&is_hps=y",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
  ...
  },
  {
    render: function(data, type, full, meta) {
      ...
      if (full.pekerjaan_dokumen_status == '0' && full.revisi_ifc == 'y' && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        ...
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi + ' - Send';
      } else if (full.pekerjaan_dokumen_status == '2') {
        var data = 'IFA - Send';
      } else if (full.pekerjaan_dokumen_status == '3' && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi + ' - Review AVP';
      } else if (full.pekerjaan_dokumen_status == '3') {
        var data = 'IFA - Review AVP';
      } else if (full.pekerjaan_dokumen_status == '4' && (full.pic == 'y' || full.picavp == 'y' || full.picvp == 'y') && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi;
      } else if (full.pekerjaan_dokumen_status == '4' && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi + ' - Approve VP';
      } else if (full.pekerjaan_dokumen_status == '4') {
        var data = 'IFA - Approve VP';
      } else if (full.pekerjaan_dokumen_status == '5' && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi;
      } else if (full.pekerjaan_dokumen_status == '5') {
        var data = 'IFA'
      } else if (full.pekerjaan_dokumen_status == '6' && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi + ' - Review AVP User';
      } else if (full.pekerjaan_dokumen_status == '6') {
        var data = 'IFA'
      } else if (full.pekerjaan_dokumen_status == '7' && (full.pekerjaan_dokumen_revisi != null && full.pekerjaan_dokumen_revisi != '')) {
        var data = 'IFA Rev ' + full.pekerjaan_dokumen_revisi + ' - Approve VP User';
...
$('#table_dokumen_ifa').DataTable({
  ...
  initComplete: function() {
    ...
    api.columns().eq(0).each(function(colIdx) {
      ...
      // },
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenIFA?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&id_user=<?= $this->input->get('id_user') ?>&is_hps=n",
    "dataSrc": ""
  },
  "columns": [
        // {
  ...
  },
  {
    "render": function(data, type, full, meta) {
            ...
            // } else if (full.is_proses == 'a' && full.vp == 'n' && full.is_hps == 'n' && full.cc == 'y') {
            //   aksi = '<center><a href="javascript:;" id="' + full.pekerjaan_dokumen_id + '" title="Aksi" onclick="fun_aksi_ifa(this.id)"><i class="fa fa-share"></i></a></center>';
            // } else if (full.is_proses == 'a' && full.vp == 'n' && full.is_hps == 'n' && full.pic == 'y') {
            //   aksi = '<center><a href="javascript:;" id="' + full.pekerjaan_dokumen_id + '" title="Aksi" onclick="fun_aksi_ifa(this.id)"><i class="fa fa-share"></i></a></center>';
            // } else if (full.vp == 'n' && full.cc == 'y' && full.pekerjaan_dokumen_status_review < 2) {
            //   aksi = '<center><a href="javascript:;" id="' + full.pekerjaan_dokumen_id + '" title="Aksi" onclick="fun_aksi_ifa_cc(this.id)"><i class="fa fa-share"></i></a></center>';
            ...
            // return aksi;
      var aksi = '';
      if (full.id_bagian == $('#session_bagian').val() && full.avp == 'y' && full.is_proses != 'y' && full.vp != 'y' && full.is_update_ifa == null) {
        aksi = '<center><a href="javascript:;" id="' + full.pekerjaan_dokumen_id + '" title="Aksi" onclick="fun_aksi_ifa(this.id)"><i class="fa fa-share"></i></a></center>';
...
$('#table_dokumen_ifa_hps').DataTable({
  ...
  initComplete: function() {
    ...
    api.columns().eq(0).each(function(colIdx) {
      ...
      // },
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenIFAHPS?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&is_hps=y&id_user=<?= $this->input->get('id_user') ?>",
    "dataSrc": ""
  },
  "columns": [
        // {
...
$('#table_dokumen_ifc').DataTable({
  ...
  initComplete: function() {
    ...
    api.columns().eq(0).each(function(colIdx) {
      ...
      // },
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenIFC?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&is_hps=n&id_user=<?= $this->input->get('id_user') ?>",
    "dataSrc": ""
  },
  "columns": [
        // {
...
$('#table_dokumen_ifc_hps').DataTable({
      ...
      // },
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenIFC?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&is_hps=y&id_user=<?= $this->input->get('id_user') ?>",
    "dataSrc": ""
  },
  "columns": [
        // {
...
$('#table_dokumen_selesai').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenSelesai?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&is_hps=n&id_user=<?= $this->input->get('id_user') ?>&id_user_cc=<?= $this->input->get('id_user_cc') ?>",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
...
$('#table_dokumen_selesai_ifa').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenSelesaiIFA?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&id_user=<?= $this->input->get('id_user') ?>&id_user_cc=<?= $this->input->get('id_user_cc') ?>&is_hps=n",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
...
$('#table_dokumen_selesai_hps').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getDokumenSelesaiHPS?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>&pekerjaan_status=<?= $this->input->get('status') ?>&is_hps=y&id_user=<?= $this->input->get('id_user') ?>&id_user_cc=<?= $this->input->get('id_user_cc') ?>",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
...
$('#id_tanggung_jawab_vp').select2({
  ...
  ajax: {
    delay: 250,
    url: '<?= base_url('project/pekerjaan_usulan/getUserListVP?id_user='.$this->input->get('id_user')) ?>',
    dataType: 'json',
    type: 'GET',
...
$('#id_user_avp').select2({
  ...
  ajax: {
    delay: 250,
    url: '<?= base_url('project/pekerjaan_langsung/getUserListAVP?pekerjaan_id=' . $this->input->get('pekerjaan_id') . '&user_id=' . $this->input->get('id_user')) ?>',
    dataType: 'json',
    type: 'GET',
    data: function(params) {
      var queryParameters = {
...
$('#id_perencana_baru').select2({
  ...
  ajax: {
    delay: 250,
    url: '<?= base_url('project/pekerjaan_usulan/getUserPengganti') ?>?id_user=<?= $this->input->get('id_user') ?>',
    dataType: 'json',
    type: 'GET',
    data: function(params) {
      var queryParameters = {
...
fun_loading();
fun_cekRevisiIFA('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>');
});
    ...
    if (result.value) {
      ...
      setTimeout(() => {
        if ($('#is_rkap').val() == '1') {
          ...
          window.location.replace('<?= base_url() ?>project/pekerjaan_langsung');
        }
      }, "1000");
    }
  });
...
function fun_reject_staf(id) {
  ...
  /* Klik Disposisi VP */
function fun_disposisi_vp(id) {
  $('#modal_vp').modal('show');
  $('#simpan_vp').show()
  $('#id_user_vp').empty();
  $.getJSON('<?= base_url('project/pekerjaan_langsung/getVPPJ') ?>', {
    pekerjaan_id: id,
  ...
  $.getJSON('<?= base_url('project/pekerjaan_langsung/getVPTerkait') ?>', {
    pekerjaan_id: id,
    user_id: '<?= $this->input->get('id_user') ?>',
  }, function(json2) {
    $.each(json2, function(index2, val2) {
      $('#' + index2).val(val2);
    });
  })
}
...
function fun_disposisi_avp(id) {
  $.getJSON('<?= base_url('project/pekerjaan_langsung/getVPAVP') ?>', {
    pekerjaan_id: id,
    user_id: '<?= $this->input->get('id_user') ?>',
  ...
  $.getJSON('<?= base_url('project/pekerjaan_langsung/getAVP') ?>', {
    pekerjaan_id: id,
    user_id: '<?= $this->input->get('id_user') ?>'
  }, function(json) {
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_avp').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
...
function fun_disposisi_avp_check_tj(id) {
  $.getJSON('<?= base_url('project/pekerjaan_langsung/getVPAVPTJ') ?>', {
    pekerjaan_id: id,
    user_id: '<?= $this->input->get('id_user') ?>',
  }, function(json) {
    $.each(json, function(index, val) {
...
function fun_progress(id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/') ?>getProgressPekerjaan?pekerjaan_id=' + id + '&id_user=<?= $this->input->get('id_user') ?>', function(json) {
    if (json && json != null) {
...
function fun_upload(id, val) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_staf').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  setTimeout(function() {
    $('#dg_document').edatagrid({
      url: '<?= base_url("project/pekerjaan_usulan/getAsetDocument?pekerjaan_dokumen_status=n&id_pekerjaan=") ?>' + id + '&pekerjaan_status=' + val + '&id_create=' + $('#session_user').val() + '&is_hps=n&id_user=<?= $this->input->get('id_user') ?>',
      saveUrl: '<?= base_url('project/pekerjaan_usulan/insertAsetDocument?id_user=') ?><?= $this->input->get('id_user') ?>',
      updateUrl: '<?= base_url('project/pekerjaan_usulan/updateAsetDocument?id_user=') ?><?= $this->input->get('id_user') ?>',
      ...
      onAdd:function(index,row){
        var up = $(this).datagrid('getEditor', {index:index,field:'urutan_proyek_nama'});
        $(up.target).textbox('setValue', row.urutan_proyek_nama);

        var sa = $(this).datagrid('getEditor', {index:index,field:'section_area_nama'});
...
function fun_upload_hps(id, val) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_staf_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  setTimeout(function() {
    $('#dg_document_hps').edatagrid({
      url: '<?= base_url("project/pekerjaan_usulan/getAsetDocument?pekerjaan_dokumen_status=n&id_pekerjaan=") ?>' + id + '&pekerjaan_status=' + val + '&id_create=' + $('#session_user').val() + '&is_hps=y&id_user=<?= $this->input->get('id_user') ?>',
      saveUrl: '<?= base_url('project/pekerjaan_usulan/insertAsetDocument?id_user=') ?><?= $this->input->get('id_user') ?>',
      updateUrl: '<?= base_url('project/pekerjaan_usulan/updateAsetDocument?id_user=') ?><?= $this->input->get('id_user') ?>',
      rowStyler: function(index, row) {
        if (row.pekerjaan_dokumen_status == '0') {
      ...
      onAdd:function(index,row){
        ...
        var kertas = $(this).datagrid('getEditor', {index: index, field: 'pekerjaan_dokumen_kertas'});
...
function fun_upload_ifc(id, val) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_staf_ifc').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  setTimeout(function() {
    $('#dg_document_ifc').edatagrid({
      url: '<?= base_url("project/pekerjaan_usulan/getAsetDocumentIFC?pekerjaan_dokumen_status=n&id_pekerjaan=") ?>' + id + '&pekerjaan_status=' + val + '&id_user=<?= $this->input->get('id_user') ?>&id_create=' + $('#session_user').val() + '&is_hps=n&id_user=<?= $this->input->get('id_user') ?>',
      saveUrl: '<?= base_url('project/pekerjaan_usulan/insertAsetDocumentIFC?id_user=') ?><?= $this->input->get('id_user') ?>',
      updateUrl: '<?= base_url('project/pekerjaan_usulan/updateAsetDocumentIFC?id_user=') ?><?= $this->input->get('id_user') ?>',
      rowStyler: function(index, row) {
        if (row.pekerjaan_dokumen_status == '0') {
          return 'background-color:#FF0000;font-weight:bold;';
        }
...
function fun_upload_ifc_hps(id, val) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_staf_ifc_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  setTimeout(function() {
    $('#dg_document_ifc_hps').edatagrid({
      ...
      url: '<?= base_url("project/pekerjaan_usulan/getAsetDocumentIFC?pekerjaan_dokumen_status=n&id_pekerjaan=") ?>' + id + '&pekerjaan_status=' + val + '&id_create=' + $('#session_user').val() + '&is_hps=y&id_user=<?= $this->input->get('id_user') ?>',
      saveUrl: '<?= base_url('project/pekerjaan_usulan/insertAsetDocumentIFC?id_user=') . $this->input->get('id_user') ?>',
      updateUrl: '<?= base_url('project/pekerjaan_usulan/updateAsetDocumentIFC?id_user=') . $this->input->get('id_user') ?>',
      rowStyler: function(index, row) {
        if (row.pekerjaan_dokumen_status == '0') {
...
function fun_approve_berjalan(id, text) {
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.getJSON('<?= base_url('project/pekerjaan_usulan/prosesApproveBerjalan') ?>?id_user=<?= $this->input->get('id_user') ?>', {
        pekerjaan_id: id,
        pekerjaan_status: $('#pekerjaan_status').val(),
        id_user_staf: $('#id_user_staf').val(),
        id_user_staf_hps: $('#id_user_staf_hps').val()
      }, function(json) {});
    }
  });
}
...
function fun_approve_berjalan_ifc(id, text) {
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.getJSON('<?= base_url('project/pekerjaan_usulan/prosesApproveBerjalanIFC?id_user=' . $this->input->get('id_user')) ?>', {
        pekerjaan_id: id,
        pekerjaan_status: $('#pekerjaan_status').val(),
        id_user_staf: $('#id_user_staf').val(),
      }, function(json) {});
...
function fun_approve_ifa(id, text) {
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.getJSON('<?= base_url('project/pekerjaan_usulan/prosesApproveIFA?id_user=' . $this->input->get('id_user')) ?>', {
        pekerjaan_id: id,
        pekerjaan_status: $('#pekerjaan_status').val(),
        id_user: '<?= $this->input->get('id_user') ?>',
      }, function(json) {});
...
$("#form_modal_vp").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/disposisiVP') ?>?id_user=<?= $this->input->get('id_user') ?>';
...
$("#form_modal_avp").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/disposisiAVP') ?>?id_user=<?= $this->input->get('id_user') ?>';
  e.preventDefault();
  Swal.fire({
...
$('#form_modal_ganti_perencana').on('submit', function(e) {
  ...
  $.ajax({
    url: '<?= base_url('project/pekerjaan_usulan/gantiPerencana') ?>?id_user=<?= $this->input->get('id_user') ?>',
    data: data,
    dataType: 'HTML',
    type: 'POST',
    processData: false,
    contentType: false,
    beforeSend: function() {
      $('#loading_form_ganti_perencana').show()
      $('#simpan_ganti_perencana').hide();
    },
    complete: function() {
...
$('#form_modal_ganti_koor').on('submit', function(e) {
  ...
  if ($('#pekerjaan_status_ganti_koor').val() == '4') {
    var url = '<?= base_url('project/pekerjaan_usulan/gantiKoor?id_user=' . $this->input->get('id_user')) ?>'
  } else if ($('#pekerjaan_status_ganti_koor').val() == '5') {
    var url = '<?= base_url('project/pekerjaan_usulan/gantiKoorPerencana?id_user=' . $this->input->get('id_user')) ?>'
  }
  ...
  $.ajax({
    ...
    contentType: false,
    beforeSend: function() {
      $('#loading_form_ganti_koor').show()
      $('#simpan_ganti_koor').hide();
    },
    complete: function() {
...
$('#simpan_progress').on('click', function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/insertProgressPekerjaan') ?>?id_user=<?= $this->input->get('id_user') ?>';
  e.preventDefault();
...
$('#edit_progress').on('click', function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/updateProgressPekerjaan') ?>?id_user=<?= $this->input->get('id_user') ?>';
  e.preventDefault();
...
$('#form_upload_dokumen').on('submit', function(e) {
  ...
  if ($('#dg_document').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    fun_approve_berjalan('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Send?');
    $('#close_dokumen').click();
  }
});
  /* Send IFA */
...
$('#form_upload_dokumen_hps').on('submit', function(e) {
  ...
  if ($('#dg_document_hps').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    fun_approve_berjalan_hps('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Send?');
...
$('#form_upload_dokumen_ifc').on('submit', function(e) {
  ...
  if ($('#dg_document_ifc').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    fun_approve_berjalan_ifc('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>', 'Apakah Anda Yakin Send?');
...
$("#form_modal_aksi").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/simpanAksi') ?>?id_user=<?= $this->input->get('id_user') ?>';
  ...
  $.ajax({
    ...
    success: function(isi) {
      ...
      fun_cekRevisi('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>');
    }
  });
});
  /* Simpan Aksi Dokumen Pekerjaan */
...
$("#form_modal_aksi_cc").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/simpanAksiCC') ?>?id_user=<?= $this->input->get('id_user') ?>';
...
$("#form_modal_aksi_staf").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/simpanAksiStaf') ?>?id_user=<?= $this->input->get('id_user') ?>';
  ...
  $.ajax({
    ...
    contentType: false,
    beforeSend: function() {
      $('#loading_form_aksi_staf').show()
      $('#simpan_aksi_staf').hide();
    },
    complete: function() {
...
$("#form_modal_aksi_ifa").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/simpanAksiIFA') ?>?id_user=<?= $this->input->get('id_user') ?>';
  ...
  $.ajax({
    ...
    contentType: false,
    beforeSend: function() {
      $('#loading_form_aksi_ifa').show()
      $('#simpan_aksi_ifa').hide();
    },
    complete: function() {
...
$("#form_modal_aksi_ifa_cc").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/simpanAksiIFACC') ?>?id_user=<?= $this->input->get('id_user') ?>';
  ...
  data.append('pekerjaan_dokumen_status_nomor', $('#pekerjaan_dokumen_status_ifa_cc').val());
  ...
  $.ajax({
    ...
    contentType: false,
    beforeSend: function() {
      $('#loading_form_aksi_ifa_cc').show()
      $('#simpan_aksi_ifa_cc').hide();
    },
    complete: function() {
...
$("#form_modal_aksi_ifc").on("submit", function(e) {
  var url = '<?= base_url('project/pekerjaan_usulan/simpanAksiIFC') ?>?id_user=<?= $this->input->get('id_user') ?>';
  ...
  $.ajax({
    ...
    contentType: false,
    beforeSend: function() {
      $('#loading_form_aksi_ifc').show()
      $('#simpan_aksi_ifc').hide();
    },
    complete: function() {
...
$('#form_modal_send_vp').on('submit', function(e) {
  e.preventDefault();
  var url = '<?= base_url('project/pekerjaan_usulan/prosesSendVP') ?>?id_user=<?= $this->input->get('id_user') ?>';
  $.ajax({
    url: url,
    data: {
    ...
    $.each(json, function(index, val) {
      if (val.total > 0 && val.total != val.total_koor) {
        ...
        $.ajax({
          ...
          dataType: 'HTML',
          beforeSend: function() {
            $('#loading_form_send_vp_koor').show()
            $('#simpan_send_vp_koor').hide();
          },
          complete: function() {
...
$('#form_modal_send_avp_ifc').on('submit', function(e) {
  e.preventDefault();
  var url = '<?= base_url('project/pekerjaan_usulan/prosesSendAVPIFC?id_user=' . $this->input->get('id_user')) ?>';
  $.ajax({
    url: url,
    data: {
    ...
    dataType: 'HTML',
    beforeSend: function() {
      $('#loading_form_send_avp_ifc').show()
      $('#simpan_send_avp_ifc').hide();
    },
    complete: function() {
...
$('#form_modal_send_vp_ifc').on('submit', function(e) {
  e.preventDefault();
  var url = '<?= base_url('project/pekerjaan_usulan/prosesSendVPIFC?id_user=') . $this->input->get('id_user') ?>';
  $.ajax({
    url: url,
    data: {
    ...
    dataType: 'HTML',
    beforeSend: function() {
      $('#loading_form_send_vp_ifc').show()
      $('#simpan_send_vp_ifc').hide();
    },
    complete: function() {
...
$('#form_modal_approve_vp').on('submit', function(e) {
  e.preventDefault();
  var url = '<?= base_url('project/pekerjaan_usulan/prosesApproveVP?id_user=') . $this->input->get('id_user') ?>';
  $.ajax({
    url: url,
    data: {
    ...
    dataType: 'HTML',
    beforeSend: function() {
      $('#loading_form_approve_vp').show()
      $('#simpan_approve_vp').hide();
    },
    complete: function() {
...
function fun_tambah_document_hps() {
  ...
  if ($('#dg_document_hps').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    var id = '<?= $this->input->get('pekerjaan_id') ?>';
    $.getJSON('<?=base_url('project/pekerjaan_usulan/getPenomoranDokumen')?>', {id_pekerjaan: id,single:'y'}, function(json, textStatus) {
...
function funcModalSendVP(id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_vp').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_vp_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
...
function funcModalSendVPKoor(id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_vp_koor').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_vp_koor_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
...
function funcModalSendVPIFC(id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_vp_ifc').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_vp_ifc_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
...
function funcModalSendAVPIFC(id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserKoorVPIFC?id_user=' . $this->input->get('id_user')) ?>', {
    pekerjaan_id: id,
    status: '<?= $this->input->get('status') ?>',
  }, function(json, result) {
    if (json.id_penanggung_jawab == 'n') {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_avp_ifc').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    pekerjaan_id: id,
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_send_avp_ifc_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
...
function funcModalApproveVP(id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_approve_vp').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + ' - ' + val.pegawai_postitle + '</option>');
    });
  });
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getUserStafVP') ?>', {
    ...
    $.each(json, function(index, val) {
      $('#' + index).val(val);
      $('#id_user_approve_vp_hps').append('<option selected value="' + val.id_user + '">' + val.pegawai_nama + '-' + val.pegawai_postitle + '</option>');
    });
  });
...
$('#simpan_dokumen_ifc_revisi').on('click', function() {
  var id = '<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $this->input->get('pekerjaan_id')) ?>';
  Swal.fire({
...
function fun_draft_dokumen(id) {
  ...
  if ($('#dg_document').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    $.ajax({
      url: '<?= base_url('project/pekerjaan_usulan/insertCCDraft?id_user=' . $this->input->get('id_user')) ?>',
...
function fun_draft_dokumen_hps(id) {
  ...
  if ($('#dg_document_hps').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    $.ajax({
      url: '<?= base_url('project/pekerjaan_usulan/insertCCDraftHPS?id_user=' . $this->input->get('id_user')) ?>',
...
function fun_draft_dokumen_ifc(id) {
  $.ajax({
    url: '<?= base_url('project/pekerjaan_usulan/insertCCDraftIFC?id_user=' . $this->input->get('id_user')) ?>',
...
function fun_draft_dokumen_ifc_hps(id) {
  ...
  if ($('#dg_document_ifc_hps').data('datagrid').data.total > 0 && 'isNewRecord' in isi_awal) {
    ...
    $.ajax({
      url: '<?= base_url('project/pekerjaan_usulan/insertCCDraftIFCHPS?id_user=' . $this->input->get('id_user')) ?>',
      data: {
...
function fun_refresh(id, isi) {
  var data = isi.split("~");
  if (parseInt(data[1]) >= '0' && parseInt(data[1]) <= '4') var aksi = 'usulan';
  else if (parseInt(data[1]) >= '5' && parseInt(data[1]) <= '7') var aksi = 'berjalan';
  else if (parseInt(data[1]) >= '8' && parseInt(data[1]) <= '10') var aksi = 'ifa';
  else if (parseInt(data[1]) >= '11' && parseInt(data[1]) <= '13') var aksi = 'ifc';
  else var aksi = 'selesai';
  window.location.replace('<?= base_url() ?>project/pekerjaan_langsung/detailPekerjaan?aksi=' + aksi + '&pekerjaan_id=' + id + '&status=' + parseInt(data[1]) + '&rkap=0&id_user=' + data[0] + '&id_user_cc=');
}

function fun_refresh_cc(id, isi) {
  var data = isi.split("~");
  if (parseInt(data[1]) >= '0' && parseInt(data[1]) <= '4') var aksi = 'usulan';
  else if (parseInt(data[1]) >= '5' && parseInt(data[1]) <= '7') var aksi = 'berjalan';
  else if (parseInt(data[1]) >= '8' && parseInt(data[1]) <= '10') var aksi = 'ifa';
  ...
  window.location.replace('<?= base_url() ?>project/pekerjaan_langsung/detailPekerjaan?aksi=selesai&pekerjaan_id=' + id + '&status=' + parseInt(data[1]) + '&rkap=0&id_user_cc=' + data[0] + '&id_user=');
}
</script>