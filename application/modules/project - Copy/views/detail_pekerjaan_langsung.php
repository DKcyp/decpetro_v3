...
$sql_selesai = $this->db->query("SELECT COUNT(*) AS total FROM dec.dec_pekerjaan_disposisi a WHERE a.id_pekerjaan = '" . $_GET['pekerjaan_id'] . "' AND CAST(pekerjaan_disposisi_status as INT) >= '12'");
$data_selesai = $sql_selesai->row_array();
                  ...
                  <div class="card-body">
                    ...
                    // CC
                      ...
                      if (isset($dataJumlah[0]['pekerjaan_disposisi_status']) && $dataJumlah[0]['pekerjaan_disposisi_status'] == '3') {
                        $total = (COUNT($data_cc) != NULL) ? 65 : 100;
                          ...
                          <li style="width: <?= $total ?>%">
                            ...
                            // CC
                              if ($dataJumlah[0]['pekerjaan_disposisi_status'] == '2') {
                                $total1 = (COUNT($data_cc) != NULL) ? 65 : 100;
                                    ...
                                    <li style="width: <?= $total1; ?>%">
                                      ...
                                      // CC
                                        if ($dataJumlah[0]['pekerjaan_disposisi_status'] == '1') {
                                          $total2 = (COUNT($data_cc) != NULL) ? 65 : 100;
    ...
    $(function() {
      ...
      // console.log(json);
        ...
        // Upload File Satf Cangun dan Sudah Diproses
          ...
          //   $('#btn_reject_staf').hide();
          //   $('#btn_ganti_perencana').hide();
          } else if (json.pekerjaan_disposisi_status == '8' && json.is_cc != null) {
          /* Approve PIC */
            $('#btn_upload').css('display', 'none');
            $('#btn_upload_hps').css('display', 'none');
            $('#btn_progress').css('display', 'none');
...
$('#table_dokumen_ifc').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getAsetDocumentIFC?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $_GET['pekerjaan_id']) ?>&pekerjaan_status=<?= $_GET['status'] ?>&is_hps=n&id_user=<?=$_GET['id_user']?>",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
...
$('#table_dokumen_ifc_hps').DataTable({
  "ajax": {
    "url": "<?= base_url('project/pekerjaan_usulan/') ?>getAsetDocumentIFC?id_pekerjaan=<?php echo preg_replace("/[^0-9^a-z^A-Z]/", "", $_GET['pekerjaan_id']) ?>&pekerjaan_status=<?= $_GET['status'] ?>&is_hps=y&id_user=<?=$_GET['id_user']?>",
    "dataSrc": ""
  },
  "columns": [{
    render: function(data, type, full, meta) {
...
function fun_upload_ifc(id, val) {
  ...
  setTimeout(function() {
    $('#dg_document_ifc').edatagrid({
      url: '<?= base_url("project/pekerjaan_usulan/getAsetDocumentIFC?pekerjaan_dokumen_status=n&id_pekerjaan=") ?>' + id + '&pekerjaan_status=' + val + '&id_user=<?= $_GET['id_user'] ?>&id_create=' + $('#session_user').val() + '&is_hps=n',
...
function fun_refresh(id, isi) {
  ...
  if (parseInt(data[1]) >= '0' && parseInt(data[1]) <= '4') var aksi = 'usulan';
    ...
    // else if (parseInt(data[1]) == '8' && data[2]!='') var aksi = 'selesai';
