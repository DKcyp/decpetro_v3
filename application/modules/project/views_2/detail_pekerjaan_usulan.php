    ...
    <div class="row" id="div_home">
      ...
      <?php elseif ($_GET['aksi'] == 'berjalan') : ?>
        ...
        <?php elseif ($_GET['aksi'] == 'ifc') : ?>
          ...
          <div class="col-lg-12">
            <div class="card">
              <?php $cek = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE id_user = '".$this->session->userdata('pegawai_nik')."' AND is_proses IS NULL AND id_pekerjaan = '".$this->input->get('pekerjaan_id')."' AND pekerjaan_disposisi_status = '".$this->input->get('status')."' AND id_penanggung_jawab = 'y'")->num_rows();
              if($cek=='0'){ ?>
                <div class="card-body" id="div_cek_proses">
              ...
              <div class="card-body">
                ...
                <button type="button" class="btn btn-danger col-1 float-end" id="btn_revisi" onclick="fun_reject_berjalan_ifc('<?= preg_replace("/[^0-9^a-z^A-Z]/", "", $_GET['pekerjaan_id']) ?>')" style="display:none;margin-left: 0.5cm;">
                  Revisi
                </button>
                <!-- Tombol -->
              </div>              
