...
class Pekerjaan_usulan extends MX_Controller
{
  ...
  public function prosesSendVP() {
    $isi = (isset($_GET['id_user'])) ? $this->db->get_where('global.global_pegawai', ['pegawai_nik' => $this->input->get('id_user')])->row_array() : $this->session->userdata();
    ...
    /* Insert CC */
      if ($this->input->get_post('id_user_send_vp')) $this->fun_insert_cc($pekerjaan_id, 'y', $this->input->get_post('id_user_send_vp'));
      if ($this->input->get_post('id_user_send_vp_hps')) $this->fun_insert_cc($pekerjaan_id, 'h', $this->input->get_post('id_user_send_vp_hps'));
    /* Insert CC */

    $this->db->set(['is_proses' => 'y'])->where(['id_user' => $isi['pegawai_nik'], 'id_pekerjaan' => $pekerjaan_id, 'pekerjaan_disposisi_status' => '6'])->update('dec.dec_pekerjaan_disposisi');

    $data_koordinator = $this->db->where(['id_pekerjaan' => $pekerjaan_id, 'id_penanggung_jawab' => 'y', 'pekerjaan_disposisi_status' => '4', 'id_user' => $isi['pegawai_nik']])->count_all_results('dec.dec_pekerjaan_disposisi');
    $jumlah_proses = $this->db->where(['id_pekerjaan' => $pekerjaan_id, 'pekerjaan_disposisi_status' => '6', 'is_proses IS NULL' => null])->count_all_results('dec.dec_pekerjaan_disposisi');
    $jumlah_proses_koor = $this->db->where(['id_pekerjaan' => $pekerjaan_id, 'pekerjaan_disposisi_status' => '6', 'is_proses' => 'y', 'id_penanggung_jawab' => 'y'])->count_all_results('dec.dec_pekerjaan_disposisi');

    $pekerjaan_status = '7';

    /* cek apakah koordinator atau bukan*/
    if ($pekerjaan_id) {
      if ($data_koordinator > '0' || ($jumlah_proses == '0' && $jumlah_proses_koor > '0')) {
        $this->M_pekerjaan->updatePekerjaan(['pekerjaan_status' => anti_inject($pekerjaan_status)], $pekerjaan_id);

        $user = $this->M_user->getUser(['pegawai_poscode' => $isi['pegawai_direct_superior']]);
        $isi_pekerjaan = $this->db->where('dec.dec_pekerjaan', ['pekerjaan_id' => $pekerjaan_id])->row_array();
  ...
  public function prosesSendVPIFC()
  {
    ...
    if ($pekerjaan_id) {
      if ($data_koordinator['total'] > '0') {
        ...
        if ($this->input->get_post('pekerjaan_status') == '8') {
          $usernya = $isi_pekerjaan['pic'];
        } else if ($this->input->get_post('pekerjaan_status') == '11' || $this->input->get_post('pekerjaan_status') == '12') {
          $vp_cangun = $this->db->query("SELECT * FROM global.global_pegawai WHERE pegawai_poscode = 'E53000000'")->row_array();
          $usernya = $vp_cangun['pegawai_nik'];
        } else {
          $usernya = $user['pegawai_nik'];
    ...
    foreach ($data_dokumen as $val_dokumen) {
      ...
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen IFC ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawai_nik']);
      } else {
        ...
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen IFC ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
  ...
  public function prosesApproveVP()
  {
    ...
    if ($pekerjaan_status == '8') {
      ...
      $kalimat = "Mohon untuk melakukan APPROVE DOKUMEN IFA pada pekerjaan ini";
  ...
  public function getPekerjaanBerjalan()
  {
    ...
    if (empty($param['id_user'])) {
      ...
      foreach ($sql_pekerjaan as $key => $value) {
        ...
        $dataMilikAVP = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi a LEFT JOIN dec.dec_pekerjaan b ON a.id_pekerjaan = b.pekerjaan_id WHERE a.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND a.id_user ='" . $session['pegawai_nik'] . "' AND pekerjaan_status = '" . $value['pekerjaan_status'] . "' AND pekerjaan_disposisi_status = '" . ($value['pekerjaan_status'] + 1) . "'")->num_rows();

        /*data per progress*/
        /*sipil*/
        $sql_bagian = "SELECT bagian_id,id_bagian,progress_jumlah FROM dec.dec_pekerjaan_progress a LEFT JOIN global.global_bagian b ON b.bagian_id = a.id_bagian";

        $sql_bagian_listin = "SELECT bagian_id,id_bagian,progress_jumlah,is_listin FROM dec.dec_pekerjaan_progress a LEFT JOIN global.global_bagian b ON b.bagian_id = a.id_bagian LEFT JOIN dec.dec_pekerjaan_disposisi c ON c.id_user = a.id_user AND a.id_pekerjaan = c.id_pekerjaan";

        $sql_user_bagian = "SELECT klasifikasi_dokumen_inisial,id_penanggung_jawab FROM global.global_klasifikasi_dokumen a LEFT JOIN dec.dec_pekerjaan_disposisi b ON a.id_pegawai = b.id_user LEFT JOIN global.global_bagian_detail c ON b.id_user = c.id_pegawai";

        $sql_sipil = $this->db->query("" . $sql_bagian . " WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND bagian_id ='1483c0882e75988626fee21c5926cc63727734a0'");
        $data_sipil = $sql_sipil->row_array();
        $jml_sipil = $sql_sipil->num_rows();

        $sql_user_sipil = $this->db->query("" . $sql_user_bagian . " WHERE b.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND b.pekerjaan_disposisi_status = '5' AND c.id_bagian = '1483c0882e75988626fee21c5926cc63727734a0'");
        $data_user_sipil = $sql_user_sipil->row_array();
        /*sipil*/

        /*proses*/
        $sql_proses = $this->db->query("" . $sql_bagian . " WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND bagian_id ='c21f86a03fdf9f7420764ac49d664415cfc942eb'");
        $data_proses = $sql_proses->row_array();
        $jml_proses = $sql_proses->num_rows();

        $sql_user_proses = $this->db->query("" . $sql_user_bagian . " WHERE b.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND b.pekerjaan_disposisi_status = '5' AND c.id_bagian = 'c21f86a03fdf9f7420764ac49d664415cfc942eb'");
        $data_user_proses = $sql_user_proses->row_array();
        /*proses*/

        /*mesin*/
        $sql_mesin = $this->db->query("" . $sql_bagian . " WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND bagian_id ='fd2aa961b30ede7622a57d42267edc5d5eae3e1b'");
        $data_mesin = $sql_mesin->row_array();
        $jml_mesin = $sql_mesin->num_rows();

        $sql_user_mesin = $this->db->query("" . $sql_user_bagian . " WHERE b.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND b.pekerjaan_disposisi_status = '5' AND c.id_bagian = 'fd2aa961b30ede7622a57d42267edc5d5eae3e1b'");
        $data_user_mesin = $sql_user_mesin->row_array();

        /*mesin*/

        /*listrik*/
        $sql_listrik = $this->db->query("" . $sql_bagian_listin . "  WHERE c.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND bagian_id ='f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND is_listin = 'L' and pekerjaan_disposisi_status = '5'");
        $data_listrik = $sql_listrik->row_array();
        $jml_listrik = $sql_listrik->num_rows();

        $sql_user_listrik = $this->db->query("" . $sql_user_bagian . " WHERE b.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND b.pekerjaan_disposisi_status = '5' AND c.id_bagian = 'f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND b.is_listin = 'L'");
        $data_user_listrik = $sql_user_listrik->row_array();
        /*listrik*/

        /*instrumen*/
        $sql_instrumen = $this->db->query("" . $sql_bagian_listin . " WHERE c.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND bagian_id ='f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND is_listin='I' and pekerjaan_disposisi_status = '5' ");
        $data_instrumen = $sql_instrumen->row_array();
        $jml_instrumen = $sql_instrumen->num_rows();

        $sql_user_instrumen = $this->db->query("" . $sql_user_bagian . " WHERE b.id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND b.pekerjaan_disposisi_status = '5' AND c.id_bagian = 'f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND b.is_listin = 'I'");
        ...
        $isi['pekerjaan_isi_sipil'] = (isset($data_user_sipil['id_penanggung_jawab']) && $data_user_sipil['id_penanggung_jawab'] == 'n') ? $isi['pekerjaan_isi_sipil'] : '<b>' . $isi['pekerjaan_isi_sipil'] . '</b>';

        // $isi['pekerjaan_isi_sipil'] = (isset($data_user_sipil['id_penanggung_jawab']) && $data_user_sipil['id_penanggung_jawab'] == 'n' && !empty($isi['pekerjaan_isi_sipil'])) ? $isi['pekerjaan_isi_sipil'] : '<b>' . $isi['pekerjaan_isi_sipil'] . '</b>';
        ...
        $isi['pekerjaan_isi_proses'] = (isset($data_user_proses['id_penanggung_jawab']) && $data_user_proses['id_penanggung_jawab'] == 'n') ? $isi['pekerjaan_isi_proses'] : '<b>' . $isi['pekerjaan_isi_proses'] . '</b>';
        ...
        $isi['pekerjaan_isi_mesin'] = (isset($data_user_mesin['id_penanggung_jawab']) && $data_user_mesin['id_penanggung_jawab'] == 'n') ? $isi['pekerjaan_isi_mesin'] : '<b>' . $isi['pekerjaan_isi_mesin'] . '</b>';
        ...
        $isi['pekerjaan_isi_listrik'] = (isset($data_user_listrik['id_penanggung_jawab']) && $data_user_listrik['id_penanggung_jawab'] == 'n') ? $isi['pekerjaan_isi_listrik'] : '<b>' . $isi['pekerjaan_isi_listrik'] . '</b>';
        ...
        $isi['pekerjaan_isi_instrumen'] = (isset($data_user_instrumen['id_penanggung_jawab']) && $data_user_instrumen['id_penanggung_jawab'] == 'n') ? $isi['pekerjaan_isi_instrumen'] : '<b>' . $isi['pekerjaan_isi_instrumen'] . '</b>';


        if ($jml_sipil > 0 && $data_sipil['bagian_id'] == '1483c0882e75988626fee21c5926cc63727734a0') {
          $isi['pekerjaan_jumlah_sipil'] = ($jml_sipil > 0) ? $jml_sipil : 0;
        } else {
        ...
        $dokumen_revisi_berjalan_rev = $this->db->query("SELECT count(*) as total FROM dec.dec_pekerjaan_dokumen WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND  pekerjaan_dokumen_status = '0'")->row_array();
        $avp_belum_proses_arr = $this->db->query("SELECT b.pegawai_nama_bag FROM dec.dec_pekerjaan_disposisi a LEFT JOIN global.global_pegawai b on a.id_user = b.pegawai_nik WHERE pekerjaan_disposisi_status = '6' AND is_proses is NULL AND id_pekerjaan = '" . $value['pekerjaan_id'] . "'")->result_array();

        $is_koor = $this->db->query("SELECT COUNT(*) as total  FROM dec.dec_pekerjaan_disposisi WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND pekerjaan_disposisi_status = '6' AND id_user = '" . $session['pegawai_nik'] . "'")->row_array();
        $dokumen_sudah_proses = 0;
        if ($is_koor['total'] > 0) {
          $dokumen_sudah_proses = $this->db->query("SELECT count(*) as total FROM dec.dec_pekerjaan_dokumen WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "'  AND is_lama !='y' AND is_proses = 'y' AND id_create_awal IN (SELECT id_user FROM dec.dec_pekerjaan_disposisi WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND pekerjaan_disposisi_status = '5' AND id_penanggung_jawab = 'y')")->row_array();
          $dokumen_sudah_proses = $dokumen_sudah_proses['total'];
        }

        $arr_avp = [];
        foreach ($avp_belum_proses_arr as $bag) {
          $arr_avp[] = $bag['pegawai_nama_bag'];
        }
        ...
        $revisi_dokumen_berjalan_lama = $dokumen_revisi_berjalan_lama['total'] + $dokumen_revisi['total'];
        $revisi_dokumen_berjalan_baru = $dokumen_revisi_berjalan_baru['total'] + $dokumen_revisi_baru['total'];
        $dokumen_revisi_berjalan_rev = $dokumen_revisi_berjalan_rev['total'];
        $dokumen_sudah_proses = $dokumen_sudah_proses;
        $avp_belum_proses = $arr_avp;
        /* tambahan */
        ...
        $proses_avp = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status='6' AND id_pekerjaan='" . $value['pekerjaan_id'] . "' AND id_user='" . $session['pegawai_nik'] . "' AND is_proses='y'")->num_rows();

        $proses_avp_belum = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi a left join global.global_bagian_detail b ON b.id_pegawai=a.id_user left join global.global_bagian c ON c.bagian_id = b.id_bagian WHERE pekerjaan_disposisi_status='6' AND id_pekerjaan='" . $value['pekerjaan_id'] . "' AND id_penanggung_jawab = 'n' and is_proses is null");
        ...
        $avp_perencana_sama = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND pekerjaan_disposisi_status IN('5','6') AND id_user = '" . $session['pegawai_nik'] . "'")->num_rows();

        $query_perencana_belum = " SELECT * FROM dec.dec_pekerjaan_disposisi a LEFT JOIN global.global_bagian_detail b ON b.id_pegawai = a.id_user WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND pekerjaan_disposisi_status = '5' AND is_proses is null ";

        $query_avp_belum = "SELECT * FROM dec.dec_pekerjaan_disposisi a LEFT JOIN global.global_bagian_detail b ON b.id_pegawai = a.id_user WHERE id_pekerjaan = '" . $value['pekerjaan_id'] . "' AND pekerjaan_disposisi_status = '6' AND is_proses is null ";

        $sql_perencana_proses_belum = $this->db->query("$query_perencana_belum AND id_bagian = 'c21f86a03fdf9f7420764ac49d664415cfc942eb'")->num_rows();
        $sql_perencana_mekanikal_belum = $this->db->query("$query_perencana_belum AND id_bagian = 'fd2aa961b30ede7622a57d42267edc5d5eae3e1b'")->num_rows();
        $sql_perencana_listrik_belum = $this->db->query("$query_perencana_belum AND id_bagian = 'f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND is_listin = 'L'")->num_rows();
        $sql_perencana_instrument_belum = $this->db->query("$query_perencana_belum AND id_bagian = 'f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND is_listin = 'I'")->num_rows();
        $sql_perencana_sipil_belum = $this->db->query("$query_perencana_belum AND id_bagian = '1483c0882e75988626fee21c5926cc63727734a0'")->num_rows();

        $sql_avp_proses_belum = $this->db->query("$query_avp_belum AND id_bagian = 'c21f86a03fdf9f7420764ac49d664415cfc942eb'")->num_rows();
        $sql_avp_mekanikal_belum = $this->db->query("$query_avp_belum AND id_bagian = 'fd2aa961b30ede7622a57d42267edc5d5eae3e1b'")->num_rows();
        $sql_avp_listrik_belum = $this->db->query("$query_avp_belum AND id_bagian = 'f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND is_listin = 'L'")->num_rows();
        $sql_avp_instrument_belum = $this->db->query("$query_avp_belum AND id_bagian = 'f683cbbca693d1a08fc010fd861b7350efa3e8d2' AND is_listin = 'I'")->num_rows();
        $sql_avp_sipil_belum = $this->db->query("$query_avp_belum AND id_bagian = '1483c0882e75988626fee21c5926cc63727734a0'")->num_rows();

        $isi['milik'] = ($dataMilik['total'] > 0 || ($value['pic'] == $session['pegawai_nik'] && ($value['pekerjaan_status'] == '0' || $value['pekerjaan_status'] == '-')) || $session['pegawai_nik'] == $this->admin_sistemnya || $dataMilikAVP > 0) ? 'y' : 'n';
        ...
        $isi['revisi_dokumen_berjalan_lama'] = $revisi_dokumen_berjalan_lama;
        $isi['revisi_dokumen_berjalan_baru'] = $revisi_dokumen_berjalan_baru;
        $isi['dokumen_revisi_berjalan_rev'] = $dokumen_revisi_berjalan_rev;
        $isi['dokumen_sudah_proses'] = $dokumen_sudah_proses;
        $isi['avp_belum_proses'] = $avp_belum_proses;
        $isi['perencana_proses'] = $proses_perencana;
        $isi['avp_proses'] = $proses_avp;
        $isi['avp_koor'] = $avp_koor;
        $isi['avp_terkait_proses'] = $avp_terkait_proses;
        $isi['bagian_nama'] = ($bidang) ? $bidang['bagian_nama'] : '';
        $isi['avp_perencana_sama'] = ($avp_perencana_sama > 1) ? 'y' : 'n';
        $isi['avp_terkait_belum'] = $proses_avp_belum->num_rows();
        $hasil = '';
        if ($proses_avp_belum->num_rows() > 1) {
          foreach ($proses_avp_belum->result_array() as $value) {
            $hasil .= $value['bagian_nama'] . ', ';
          }
          $isi['avp_terkait_bagian'] = $hasil;
        } else {
          $isi['avp_terkait_bagian'] = ($proses_avp_belum->row_array()) ? $proses_avp_belum->row_array()['bagian_nama'] : '';
        }
        $isi['perencana_proses_belum'] = $sql_perencana_proses_belum;
        $isi['perencana_mesin_belum'] = $sql_perencana_mekanikal_belum;
        $isi['perencana_listrik_belum'] = $sql_perencana_listrik_belum;
        $isi['perencana_instrumen_belum'] = $sql_perencana_instrument_belum;
        $isi['perencana_sipil_belum'] = $sql_perencana_sipil_belum;

        $isi['avp_proses_belum'] = $sql_avp_proses_belum;
        $isi['avp_mesin_belum'] = $sql_avp_mekanikal_belum;
        $isi['avp_listrik_belum'] = $sql_avp_listrik_belum;
        $isi['avp_instrumen_belum'] = $sql_avp_instrument_belum;
        $isi['avp_sipil_belum'] = $sql_avp_sipil_belum;
  ...
  public function prosesReview()
  {
    ...
    /* Pekerjaan */
    $pekerjaan_status = ($this->input->get('pekerjaan_status') == '') ? '1' : $this->input->get('pekerjaan_status') + 1;
  ...
  public function prosesApprove()
  {
    ...
    if ($this->input->get('id_user')) {
      $isi = $this->db->get_where('global.global_pegawai', ['pegawai_nik' => $this->input->get('id_user')])->row_array();
    } else {
      $isi = $this->session->userdata();
    }
  ...
  {
    $user = ($this->input->get('id_user')) ? $this->db->get_where('global.global_pegawai', array('pegawai_nik' => $this->input->get('id_user')))->row_array() : $this->session->userdata();
    ...
    if ($pekerjaan_id) {
      ...
      sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text_log);
      sendNotif($pekerjaan_id, $dari, $tujuan, $text_log);
      dblog('I',  $pekerjaan_id, $text_log, $user['pegawai_nik']);
      tasklog($pekerjaan_id, $data['pekerjaan_status'], $tujuan, $text_log, 'n');
    }
    /* Pekerjaan */
  ...
  public function prosesRejectAVP()
  {
    ...
    if ($data_pekerjaan['id_penanggung_jawab'] == 'y') {
      ...
      sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
      sendNotif($pekerjaan_id, $dari, $tujuan, $text);
      dblog('I',  $pekerjaan_id, $text, $user['pegawai_nik']);
      tasklog($pekerjaan_id, $param_disposisi['pekerjaan_status'], $tujuan, $text, 'n');
      ...
      sendNotif($pekerjaan_id, $dari, $tujuan, $text);
      dblog('I',  $pekerjaan_id, $text, $user['pegawai_nik']);
      tasklog($pekerjaan_id, '3', $tujuan, $text, 'n');
      /* Notifikasi DOF */
  ...
  public function disposisiVP()
  {
    ...
    /* Disposisi */
    $param_user['pegawai_nik'] = $this->input->post('id_tanggung_jawab_vp');
    $data_user = $this->M_user->getUser($param_user);

    $data_disposisi['pekerjaan_disposisi_id'] = create_id();
    $data_disposisi['pekerjaan_disposisi_waktu'] = date("Y-m-d H:i:s");
    if ($data_user['pegawai_pgs_id']) {
      $data_disposisi['id_user'] = $data_user['pegawai_pgs_nik'];
      $data_disposisi['id_user_asli'] = $data_user['pegawai_nik'];
      $data_disposisi['is_pgs'] = 'y';

      /* CC PGS */
      $data_disposisi_doc['pekerjaan_disposisi_id'] = create_id();
      $data_disposisi_doc['pekerjaan_disposisi_waktu'] = date('Y-m-d H:i:s');
      $data_disposisi_doc['id_user'] = $data_user['pegawai_nik'];
      $data_disposisi_doc['id_pekerjaan'] = $pekerjaan_id;
      $data_disposisi_doc['pekerjaan_disposisi_status'] = anti_inject('8');
      $data_disposisi_doc['id_penanggung_jawab'] = anti_inject('n');
      $data_disposisi_doc['is_cc'] = anti_inject('y');
      ...
      $data_cc = $this->db->get_where('global.global_pegawai', ['pegawai_nik' => $data_user['pegawai_nik']])->row_array();
      $tujuan = $data_cc['pegawai_nik'];
      $tujuan_nama = $data_cc['pegawai_nama'];
      $kalimat = "Pekerjaan telah di CC kepada anda";
      dblog('I',  $pekerjaan_id, 'Pekerjaan Telah di CC ke ' . $data_cc['pegawai_nama'] . ' Sebagai CC HPS', $isi['pegawai_nik']);
      ...
      $data_disposisi['id_user'] = $data_user['pegawai_nik'];
    }
    $data_disposisi['id_pekerjaan'] = anti_inject($pekerjaan_id);
    $data_disposisi['pekerjaan_disposisi_status'] = anti_inject($pekerjaan_status);
    ...
    /*Notifikasi*/
    if ($data_user['pegawai_pgs_id']) {
      $nik = $data_user['pegawai_pgs_nik'];
      $nama =  $data_user['pegawai_pgs_nama'] . '- PGS';
    } else {
      $nik = $data_user['pegawai_nik'];
      $nama =  $data_user['pegawai_nama'];
    }
    $tujuan = $nik;
    $tujuan_nama = $nama;
    $text = "Mohon untuk melakukan REVIEW dan DISPOSISI pada pekerjaan ini";
    dblog('I',  $pekerjaan_id, 'Pekerjaan Telah di Disposisikan ke ' . $nama . ' Sebagai AVP Koordinator', $isi['pegawai_nik']);
    tasklog($pekerjaan_id, $data_disposisi['pekerjaan_disposisi_status'], $nik, $text, 'n');
    sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
    sendNotif($pekerjaan_id, $isi['pegawai_nik'], $tujuan, $text);
    /*Notifikasi*/
    ...
    if ($this->input->post('id_user_vp')) {
      ...
      foreach ($User as $key => $id_user) {
        $param_user['pegawai_nik'] = $id_user;
        $data_user = $this->M_user->getUser($param_user);
        $data_disposisi['pekerjaan_disposisi_id'] = create_id();
        $data_disposisi['pekerjaan_disposisi_waktu'] = date("Y-m-d H:i:s");
        if ($data_user['pegawai_pgs_id']) {
        ...
        $tujuan_nama = $nama;
        $text = "Mohon untuk melakukan REVIEW dan DISPOSISI pada pekerjaan ini";
        dblog('I',  $pekerjaan_id, 'Pekerjaan Telah di Disposisikan ke ' . $nama . ' Sebagai AVP Terkait', $isi['pegawai_nik']);
        tasklog($pekerjaan_id, $data_disposisi['pekerjaan_disposisi_status'], $nik, $text, 'n');
        sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
        sendNotif($pekerjaan_id, $isi['pegawai_nik'], $tujuan, $text);
        /*Notikasi*/
  ...
  {
    $isi = ($this->input->get('id_user')) ? $this->db->get_where('global.global_pegawai', ['pegawai_nik' => $this->input->get('id_user')])->row_array() : $this->session->userdata();
    ...
    if ($this->input->post('id_user_avp')) {
      $param_user['pegawai_nik'] = $this->input->post('id_user_avp');
      $data_user = $this->M_user->getUser($param_user);
      ...
      dblog('I',  $pekerjaan_id, 'Pekerjaan Telah di Disposisikan Ke ' . $nama . ' Sebagai Perencana', $isi['pegawai_nik']);
      tasklog($pekerjaan_id, $data_disposisi['pekerjaan_disposisi_status'], $nik, $text, 'n');
      sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
      sendNotif($pekerjaan_id, $isi['pegawai_nik'], $tujuan, $text);
      /*Notifikas*/
    ...
    if ($this->input->post('id_user_avp_instrumen')) {
      $param_user['pegawai_nik'] = $this->input->post('id_user_avp_instrumen');
      $data_user = $this->M_user->getUser($param_user);
    ...
    if ($this->input->post('id_user_avp_listrik')) {
      $param_user['pegawai_nik'] = $this->input->post('id_user_avp_listrik');
      $data_user = $this->M_user->getUser($param_user);
    ...
    $this->prosesGanttChart($pekerjaan_id, $data_disposisi['id_user']);
  }
  /* Disposisi AVP */
  ...
  public function prosesApproveBerjalan()
  {
    ...
    foreach ($data_dokumen as $val_dokumen) {
      ...
      dblog('I', $this->input->get_post('pekerjaan_id'), 'Dokumen IFA ' . $data_dokumen_isi['pekerjaan_template_nama'] . ' - ' . $data_dokumen_isi['pekerjaan_dokumen_nama'] . ' Telah  DiSend ', $isi['pegawai_nik']);
    }
    ...
    foreach ($data_dokumen_hps as $val_dokumen) {
      ...
      dblog('I', $this->input->get_post('pekerjaan_id'), 'Dokumen IFA ' . $data_dokumen_hps_isi['pekerjaan_template_nama'] . ' - ' . $data_dokumen_hps_isi['pekerjaan_dokumen_nama'] . ' Telah  DiSend ', $isi['pegawai_nik']);
    }
    ...
    $tujuan = $users['pegawai_nik'];
    $tujuan_nama = $users['pegawai_nama'];
    $text = "Mohon untuk melakukan REVIEW DOKUMEN IFA pada pekerjaan ini";
  ...
  public function prosesApproveBerjalanIFC()
  {
    ...
    foreach ($data_dokumen_send as $val_dokumen) {
      ...
      if (!empty($dokumen_ada) && ($dokumen_ada['pekerjaan_dokumen_nama'] == $val_dokumen['pekerjaan_dokumen_nama'] && $dokumen_ada['id_pekerjaan_template'] == $val_dokumen['id_pekerjaan_template'] && $dokumen_ada['id_create_awal'] == $val_dokumen['id_create_awal'])) {
        ...
        dblog('I', $this->input->get_post('pekerjaan_id'), 'Dokumen IFC ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
    }
...
*/
    ...
    $tujuan = $users['pegawai_nik'];
    $tujuan_nama = $users['pegawai_nama'];
    $kalimat = "Mohon untuk melakukan REVIEW DOKUMEN IFC pada pekerjaan ini";
        ...
        foreach ($data_revisi as $key => $value) {
          ...
          foreach ($disposisi as $value2) {
            ...
            foreach ($dataStaf as $valueStaf) :
              ...
              $text = "Dokumen IFA anda telah di Revisi VP";
              /*notif ke dof*/
              tasklog($pekerjaan_id, '5', $valueStaf['pegawai_nik'], $text, 'n');
              sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
              sendNotif($pekerjaan_id, $dari, $tujuan, $text);
            endforeach;
          }
  ...
  public function prosesRejectBerjalanIFC()
  {
    ...
    if ($data_avp_reject) {
      ...
      foreach ($data_staf as $val_staf) {
        ...
        sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
        sendNotif($pekerjaan_id, $dari, $tujuan, $text);
        tasklog($pekerjaan_id, 11, $tujuan, $text, 'n');
      }
    ...
    dblog('I',  $pekerjaan_id, 'Pekerjaan Telah di Revisi Oleh Cangun', $session['pegawai_nik']);
  }


  /* Approve Pekerjaan IFA */
  public function prosesApproveIFALama()
  {
    if ($this->input->get('id_user')) {
      $sql_user = $this->db->query("SELECT * FROM global.global_pegawai WHERE pegawai_nik = '" . $this->input->get('id_user') . "'");
      $isi = $sql_user->row_array();
    ...
    foreach ($data_dokumen_non as $val_dokumen) {
      ...
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        dblog('I', $pekerjaan_id, 'Dokumen IFA ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data_dok['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawai_nik']);
      } else {
        ...
        dblog('I', $pekerjaan_id, 'Dokumen IFA ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
      $param_lama['is_lama'] = 'y';
      $param_id = $val_dokumen['pekerjaan_dokumen_id'];
      $this->M_pekerjaan->editAksi($param_lama, $param_id);
  ...
  public function prosesApproveIFA()
  {
    ...
    $jabatan = substr($isi['pegawai_jabatan'], 0, 2);
    ...
    if ($jabatan == '20') {
      $pekerjaan_id = $this->input->get_post('pekerjaan_id');
      $data['pekerjaan_status'] = '11';
      $this->M_pekerjaan->updatePekerjaan($data, $pekerjaan_id);
      dblog('I',  $pekerjaan_id, 'Pekerjaan IFA Telah di Approve VP Peminta Jasa', $isi['pegawai_nik']);

      $sql_petugas = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status = '5' AND id_pekerjaan = '" . $pekerjaan_id . "'");
      $data_petugas = $sql_petugas->result_array();

      foreach ($data_petugas as $val_petugas) {
        $data_disposisi['pekerjaan_disposisi_id'] = create_id();
        $data_disposisi['pekerjaan_disposisi_waktu'] = date('Y-m-d H:i:s');
        $data_disposisi['id_user'] = $val_petugas['id_user'];
        $data_disposisi['id_pekerjaan'] = $pekerjaan_id;
        $data_disposisi['pekerjaan_disposisi_status'] = '11';
        $data_disposisi['id_penanggung_jawab'] = $val_petugas['id_penanggung_jawab'];
        $this->M_pekerjaan->insertPekerjaanDisposisi($data_disposisi);

        $user_disposisi = $this->db->get_where('global.global_pegawai', array('pegawai_nik' => $val_petugas['id_user']))->row_array();
        $tujuan = $user_disposisi['pegawai_nik'];
        $tujuan_nama = $user_disposisi['pegawai_nama'];
        $kalimat = "Mohon untuk melakukan PROSES UPLOAD DOKUMEN pada pekerjaan ini";
        sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $kalimat);
        sendNotif($pekerjaan_id, $isi['pegawai_nik'], $tujuan, $kalimat);
      }
      ...
      $pekerjaan_id = $this->input->get_post('pekerjaan_id');
      $data['pekerjaan_status'] = 10;
      $this->M_pekerjaan->updatePekerjaan($data, $pekerjaan_id);
      dblog('I',  $pekerjaan_id, 'Pekerjaan IFA Telah di Review AVP Peminta Jasa', $isi['pegawai_nik']);

      $sql_vp = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status = '2' AND id_pekerjaan = '" . $pekerjaan_id . "'");
      $data_vp = $sql_vp->row_array();

      $data_disposisi = [
        'pekerjaan_disposisi_id' => create_id(),
        'pekerjaan_disposisi_waktu' => date("Y-m-d H:i:s"),
        'id_user' => $data_vp['id_user'],
        'id_pekerjaan' => $pekerjaan_id,
        'pekerjaan_disposisi_status' => 10,
        'id_penanggung_jawab' => '',
      ];
      $this->M_pekerjaan->insertPekerjaanDisposisi($data_disposisi);

      $user_disposisi = $this->db->get_where('global.global_pegawai', array('pegawai_nik' => $data_vp['id_user']))->row_array();
      $tujuan = $user_disposisi['pegawai_nik'];
      $tujuan_nama = $user_disposisi['pegawai_nama'];
      $kalimat = "Mohon untuk melakukan PROSES IFC pada pekerjaan ini";
      sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $kalimat);
      sendNotif($pekerjaan_id, $isi['pegawai_nik'], $tujuan, $kalimat);
      /*jika selain diatas*/
    } else {
      ...
      $data['pekerjaan_status'] = 9;
      $this->M_pekerjaan->updatePekerjaan($data, $pekerjaan_id);
      dblog('I',  $pekerjaan_id, 'Pekerjaan IFA Telah di Approve PIC Peminta Jasa', $isi['pegawai_nik']);
      ...
      $data_disposisi = [
        ...
        'id_user' => $data_avp['id_user'],
        'id_pekerjaan' => $pekerjaan_id,
        'pekerjaan_disposisi_status' => 9,
        'id_penanggung_jawab' => $data_avp['id_penanggung_jawab'],
      ];
    ...
    foreach ($data_dokumen_non as $val_dokumen) {
      ...
      $data_dok['pekerjaan_dokumen_keterangan'] = $val_dokumen['pekerjaan_dokumen_keterangan'];
      $data_dok['id_create'] = $isi['pegawai_nik'];
      $data_dok['is_proses'] = $is_proses;
      $data_dok['id_create_awal'] = $data_dokumen['id_create_awal'];
      $data_dok['pekerjaan_dokumen_waktu'] = date('Y-m-d H:i:s');
      $data_dok['pekerjaan_dokumen_waktu_update'] = date('Y-m-d H:i:s');
      $this->M_pekerjaan->simpanAksiIFASama($data_dok);
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        dblog('I', $pekerjaan_id, 'Dokumen IFC ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data_dok['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawai_nik']);
      } else {
        $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $data_dok['pekerjaan_dokumen_id'] . "'");
        dblog('I', $pekerjaan_id, 'Dokumen IFC ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
      $param_lama['is_lama'] = 'y';
      $param_id = $val_dokumen['pekerjaan_dokumen_id'];
      $this->M_pekerjaan->editAksi($param_lama, $param_id);
  ...
  public function prosesApproveIFAAVP()
  {
    ...
    foreach ($data_dokumen_hps as $val_dokumen) {
      ...
      $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $data_dok['pekerjaan_dokumen_id'] . "'");
      /*if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
								dblog('I', $pekerjaan_id, 'Dokumen ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data_dok['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawai_nik']);
							} else {
								$this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '".date('Y-m-d H:i:s')."' WHERE pekerjaan_dokumen_id = '".$data['pekerjaan_dokumen_id']."' AND pekerjaan_dokumen_status >='4'");
  ...
  public function prosesRejectIFA()
  {
      ...
      foreach ($data_revisi as $key => $value) {
        ...
        foreach ($disposisi as $value) {
          ...
          foreach ($dataStaf as $valueStaf) :
            ...
            $tujuan = $valueStaf['pegawai_nik'];
            $tujuan_nama = $valueStaf['pegawai_nama'];
            $text = 'Pekerjaan telah diajukan revisi oleh User dengan Alasan : <span style="color:red">' . $this->input->get_post('pekerjaan_note') . '</span>';
            sendWA($pekerjaan_id, $tujuan, $tujuan_nama, $text);
            sendNotif($pekerjaan_id, $dari, $tujuan, $text);
            tasklog($pekerjaan_id, 5, $tujuan, $text, 'n');
    ...
    $note = "<span style='color:red'>Revisi - " . $this->input->get_post('pekerjaan_note') . "</span>";
    $this->db->query("UPDATE dec.dec_pekerjaan SET pekerjaan_status ='5',pekerjaan_note = '" . anti_inject($note) . "' WHERE pekerjaan_id = '" . $this->input->get('pekerjaan_id') . "'");
    /* Pekerjaan */
  ...
  public function simpanAksi()
  {
    ...
    if ($NewImageName == null) {
      ...
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        ...
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen IFA ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawai_nik']);
      } else {
        $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $data['pekerjaan_dokumen_id'] . "' AND pekerjaan_dokumen_status >= '4'");
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen IFA ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
    } else {
      $data['pekerjaan_dokumen_id_temp'] = anti_inject($this->input->post('pekerjaan_dokumen_id'));
      $data['pekerjaan_dokumen_id'] = anti_inject(create_id());
  ...
  public function simpanAksiCC()
  {
    ...
    if ($NewImageName == null) {
      ...
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        ...
        $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $data['pekerjaan_dokumen_id'] . "' AND pekerjaan_dokumen_status >= '4'");
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiCheck CC', $isi['pegawai_nik']);
      }
    } else {
      $data['pekerjaan_dokumen_id_temp'] = anti_inject($this->input->post('pekerjaan_dokumen_id'));
      $data['pekerjaan_dokumen_id'] = create_id();
    ...
    /* Insert */
    $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu_update = '" . date('Y-m-d H:i:s') . "',is_review = 'y',is_lama = 'y',pekerjaan_dokumen_revisi = '' WHERE pekerjaan_dokumen_id = '" . $data['pekerjaan_dokumen_id'] . "'");
  }
  /* Aksi Approve / Reject Dokumen CC*/
  ...
  public function simpanAksiIFACC()
  {
    ...
    if ($NewImageName == null) {
      ...
      $this->M_pekerjaan->simpanAksiCC($data);
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen IFA ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawa_nik']);
      } else {
        $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $data['pekerjaan_dokumen_id'] . "' AND pekerjaan_dokumen_status >= '4'");
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen IFA ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
  ...
  public function simpanAksiTransmital()
  {
    ...
    if ($NewImageName == null) {
      ...
      $this->M_pekerjaan->simpanAksiSama($data);
      if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen Kontraktor ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  Direvisi dengan Alasan <span style="color:red">' . $data['pekerjaan_dokumen_keterangan'] . '</span>', $isi['pegawai_nik']);
      } else {
        $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $data['pekerjaan_dokumen_id'] . "' AND pekerjaan_dokumen_status >= '4'");
        dblog('I', $data_status['id_pekerjaan'], 'Dokumen Kontraktor ' . $data_dokumen['pekerjaan_template_nama'] . ' - ' . $data_dokumen['pekerjaan_dokumen_nama'] . ' Telah  DiApprove', $isi['pegawai_nik']);
      }
    } else {
      $data['pekerjaan_dokumen_id_temp'] = anti_inject($this->input->post('pekerjaan_dokumen_id'));
      $data['pekerjaan_dokumen_id'] = create_id();
  ...
  public function getListUser()
  {
    ...
    $param['q'] = $this->input->get('q');
    $param['pegawai_jabatan'] = $this->input->get('pegawai_jabatan');
    $param['pegawai_poscode'] = $this->input->get('pegawai_poscode');
    $param['pegawai_direct_superior'] = $this->input->get('pegawai_direct_superior');
    $param['pegawai_jabatan'] = $this->input->get('pegawai_jabatan');
    if ($this->input->get('pegawai_direct_superior') != 'E53000000') {
      $param['bagian_nama'] = ($this->input->get('bagian_nama')) ? $this->input->get('bagian_nama') : $session['pegawai_nama_bag'];
    }
    if ($this->input->get('perencana')) {
      $param['perencana'] = $this->input->get('perencana');
      $param['pegawai_poscode_perencana'] = $session['pegawai_poscode'];
    }
    $data = $this->M_user->getUserSelect2($param);
    foreach ($data as $key => $value) {
      if ($value['pegawai_pgs_nik'] != '') {
        array_push($list['results'], [
          'id' => $value['pegawai_nik'],
          'text' => $value['pegawai_pgs_nama'] . ' - PGS ' . $value['pegawai_postitle'],
        ]);
      } else {
        array_push($list['results'], [
          'id' => $value['pegawai_nik'],
  ...
  public function getListUserById()
  {
    $list['results'] = array();
    ...
    $data_pegawai = $this->M_user->getUser($param);

    if ($data_pegawai['pegawai_pgs_id']) {
      $data['id'] = $data_pegawai['pegawai_nik'];
      $data['text'] = $data_pegawai['pegawai_pgs_nama'] . ' - PGS ' . $data_pegawai['pegawai_postitle'];
    } else {
      $data['id'] = $data_pegawai['pegawai_nik'];
  ...
  public function getUserlistVPAllDep()
  {
    $isi = $this->session->userdata();
    $list['results'] = array();
    $param['pegawai_jabatan'] = '20f';
    $param['q'] = $this->input->get_post('q');
  ...
  public function getUserStaf()
  {
    ...
    if ($user['pegawai_unit_id'] != 'E53000') {
      $param['pegawai_unit_id'] = $user['pegawai_unit_id'];
    }

    $param['pegawai_nama'] = $this->input->get('pegawai_nama');

    $data = $this->M_pekerjaan->getUserStaf($param);
  ...
  public function getPekerjaan()
  {
    ...
    if (
      ...
      ($isi['pegawai_poscode'] == 'E53300000' ||
        ...
        $isi['pegawai_poscode'] == '283000000' ||
        $isi['pegawai_poscode'] == '283200000' ||
        $isi['pegawai_poscode'] == '283300000' ||
        $isi['pegawai_poscode'] == '283100000' ||
        $isi['pegawai_poscode'] == 'E53410041A' ||
        $isi['pegawai_poscode'] == '283400000')
    ) {
  ...
  public function insertAsetDocument()
  {
    ...
    if ($this->input->get_post('is_hps') == 'y') {
      $template_nama = '142';
    } else {
      $template_nama = $this->input->get_post('pekerjaan_template_nama');
    }
    /* Dokumen Penomoran */
    ...
    /* Template */
    $sql_template = $this->db->query("SELECT * FROM dec.dec_pekerjaan_template WHERE pekerjaan_template_id = '" . $template_nama . "'");
    ...
    if ($this->input->get_post('is_hps') == 'y') {
      $data['id_pekerjaan_template'] = '142';
      $data['pekerjaan_dokumen_jenis'] = 'Dokumen';
      $data['pekerjaan_dokumen_kertas'] = 'A4';
      $data['pekerjaan_dokumen_orientasi'] = 'Potrait';
    }
  ...
  public function gantiPerencana()
  {
    ...
    $param['pekerjaan_disposisi_id'] = create_id();
    $param['pekerjaan_disposisi_waktu'] = date('Y-m-d H:i:s');
    $param['id_user'] = $this->input->post('id_user_perencana');
  ...
  public function lihatDokumen()
  {
    ...
    if (isset($_GET['is_transmital']) && $_GET['is_transmital'] == '1') {
      $sql_dokumen = $this->db->query("SELECT * FROM dec.dec_pekerjaan_dokumen_transmital a WHERE pekerjaan_dokumen_id = '" . $_GET['id'] . "'");
      $dokumen = $sql_dokumen->row_array();
      $judul = ($dokumen['pekerjaan_dokumen_awal'] == 'y') ? $dokumen['pekerjaan_dokumen_nama'] :  $dokumen['pekerjaan_dokumen_nama'];
    } else {
  ...
  public function getDokumenBerjalan()
  {
    ...
    if ($dataUser['total'] > 0 || $user['pegawai_nik'] == $this->admin_sistemnya || $this->id_bagiannya != '0') {
      ...
      foreach ($isi as $value) {
        ...
        $dataAVPBelumReview = $this->db->query("SELECT * FROM global.global_bagian_detail a LEFT JOIN dec.dec_pekerjaan_disposisi b ON b.id_user = a.id_pegawai WHERE id_bagian = '" . $value['id_bagian'] . "' AND id_user = '" . $value['id_create'] . "' AND id_pekerjaan = '" . $this->input->get('id_pekerjaan') . "' AND pekerjaan_disposisi_status = '6' AND (is_proses!='y' OR is_proses is null) ")->num_rows();

        $dataVPBelumApprove = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi b  WHERE TRUE AND id_user = '" . $value['id_create'] . "' AND id_pekerjaan = '" . $this->input->get('id_pekerjaan') . "' AND pekerjaan_disposisi_status = '7' AND (is_proses!='y' OR is_proses is null) ")->result_array();

        $dataCC = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status = '8' AND id_pekerjaan = '" . $this->input->get('id_pekerjaan') . "' AND id_user = '" . $user['pegawai_nik'] . "' ")->num_rows();

        $value['avp'] = ($dataAVP['total'] > 0) ? 'y' : 'n';
        $value['vp'] = ($dataVP['total'] > 0) ? 'y' : 'n';
        $value['avp_belum_review'] = ($dataAVPBelumReview > 0) ? 'y' : 'n';
        $value['vp_belum_approve'] = ($dataVPBelumApprove > 0) ? 'y' : 'n';
        $value['cc'] = ($dataCC > 0) ? 'y' : 'n';
  ...
  public function getPenomoranDokumen()
  {
    ...
    $data_perencana = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE id_user = '" . $session['pegawai_nik'] . "' AND pekerjaan_disposisi_status IN ('5','11') AND is_proses IS NULL AND id_pekerjaan = '" . $this->input->get('id_pekerjaan') . "'")->num_rows();
    ...
    if ($this->input->get('single')) {
      ...
      foreach ($data as $value) {
        foreach ($value as $key => $val) {
          $isi[$key] = $val;
          $isi['perencana'] = ($data_perencana > 0) ? 'y' : 'n';
        }
        array_push($datanya, $isi);
      }
    }
    echo json_encode($datanya);
  }
  ...
  /*status dokumen ifa*/
  public function getStatusDokumenIFA()
  {
    $isi = [];

    $dokumen_ifa = $this->db->query("select * from dec.dec_pekerjaan_dokumen where id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' and is_lama = 'n' and is_hps = 'n' and pekerjaan_dokumen_status = '4' ")->num_rows();
    $dokumen_ifa_avp = $this->db->query("select * from dec.dec_pekerjaan_dokumen where id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' and is_lama = 'n' and is_hps = 'n' and pekerjaan_dokumen_status = '5' ")->num_rows();
    $dokumen_ifa_vp = $this->db->query("select * from dec.dec_pekerjaan_dokumen where id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' and is_lama = 'n' and is_hps = 'n' and pekerjaan_dokumen_status = '6' ")->num_rows();

    $dokumen_revisi = $this->db->query("select * from dec.dec_pekerjaan_dokumen where id_pekerjaan = '" . $this->input->get('pekerjaan_id') . "' and is_lama = 'n' and is_hps = 'n' and pekerjaan_dokumen_status = '0' ")->num_rows();

    switch ($this->input->get('pekerjaan_status')) {
      case '8':
        $status_dokumen = '4';
        $isi['dokumen_ifa'] = $dokumen_ifa;
        break;
      case '9':
        $status_dokumen = '5';
        $isi['dokumen_ifa_avp'] = $dokumen_ifa_avp;
        break;
      case '10':
        $status_dokumen = '6';
        $isi['dokumen_ifa_vp'] = $dokumen_ifa_vp;
        break;
      default:
        break;
    }
    ...
    $isi['dokumen_revisi'] = $dokumen_revisi;

    echo json_encode($isi);
  }
  /*status dokumen ifa*/

  public function prosesGanttChart($id_pekerjaan, $id_user) {
    $dataPekerjaan = $this->db->get_where('dec.dec_pekerjaan', ['pekerjaan_id' => $id_pekerjaan])->row_array();
    $dataDisposisi = $this->db->get_where('dec.dec_pekerjaan_disposisi', ['id_pekerjaan' => $id_pekerjaan, 'id_user' => $id_user, 'pekerjaan_disposisi_status' => '5'])->row_array();

    if ($dataPekerjaan['pekerjaan_prioritas'] == '1') {
      $dataDisposisiUser = $this->db->order_by('pekerjaan_disposisi_waktu_finish', 'desc')->get_where('dec.dec_pekerjaan_disposisi', ['id_user' => $id_user, 'pekerjaan_disposisi_status' => '5', 'pekerjaan_disposisi_waktu_finish IS NOT NULL' => null])->row_array();

      $awal = ($dataDisposisiUser && $dataDisposisiUser['pekerjaan_disposisi_waktu_finish'] > date('Y-m-d')) ? date('Y-m-d', strtotime($dataDisposisiUser['pekerjaan_disposisi_waktu_finish'] . '+1 day')) : date('Y-m-d');

      $a = 0;
      $tgl = strtotime($awal);
      while ($a < $dataDisposisi['pekerjaan_disposisi_durasi']) {
        // Jika hari tersebut adalah hari kerja (Senin hingga Jumat)
        if (date('N', $tgl) < 6) {
          $a++;
        }
        // Hanya tambahkan hari jika kita masih membutuhkan lebih banyak hari kerja
        if ($a < $dataDisposisi['pekerjaan_disposisi_durasi']) {
          $tgl = strtotime('+1 day', $tgl);
        }
      }

      $akhir = date('Y-m-d', $tgl);

      $this->db->where('pekerjaan_disposisi_id', $dataDisposisi['pekerjaan_disposisi_id'])->update('dec.dec_pekerjaan_disposisi', ['pekerjaan_disposisi_waktu_start' => $awal, 'pekerjaan_disposisi_waktu_finish' => $akhir]);

      $dataDisposisiTerakhir = $this->db->select('min(pekerjaan_disposisi_waktu_start) AS awal, max(pekerjaan_disposisi_waktu_finish) AS akhir')->get_where('dec.dec_pekerjaan_disposisi', ['id_pekerjaan' => $id_pekerjaan, 'pekerjaan_disposisi_status' => '5'])->row_array();

      $this->db->where('pekerjaan_id', $id_pekerjaan)->update('dec.dec_pekerjaan', ['pekerjaan_estimasi_mulai' => $dataDisposisiTerakhir['awal'], 'pekerjaan_estimasi_selesai' => $dataDisposisiTerakhir['akhir']]);
    } else {
      $dataDisposisiUser = $this->db->order_by('pekerjaan_disposisi_waktu_finish', 'desc')->get_where('dec.dec_pekerjaan_disposisi', ['id_user' => $id_user, 'pekerjaan_disposisi_status' => '5', 'pekerjaan_disposisi_prioritas' => '2', 'pekerjaan_disposisi_waktu_finish IS NOT NULL' => null])->row_array();
      ...
      $dataDisposisiIrisan = $this->db->get_where('dec.dec_pekerjaan_disposisi', ['id_user' => $id_user, 'pekerjaan_disposisi_status' => '5', 'pekerjaan_disposisi_prioritas' => '1', "(pekerjaan_disposisi_waktu_start > '" . date('Y-m-d') . "' OR (pekerjaan_disposisi_waktu_start <= '" . date('Y-m-d') . "' AND pekerjaan_disposisi_waktu_finish >= '" . date('Y-m-d') . "'))" => null])->result_array();

      foreach ($dataDisposisiIrisan as $value) {
        $awal = date('Y-m-d', strtotime($akhir . '+1 day'));

        $a = 0;
        $tgl = strtotime($awal);
        while ($a < ($value['pekerjaan_disposisi_durasi'] - 1)) {
          if (date('N', $tgl) < 6) $a++;
          $tgl = strtotime('+1 day', $tgl);
        }
        ...
        $this->db->where('pekerjaan_disposisi_id', $value['pekerjaan_disposisi_id'])->update('dec.dec_pekerjaan_disposisi', ['pekerjaan_disposisi_waktu_start' => $awal, 'pekerjaan_disposisi_waktu_finish' => $akhir]);

        $dataDisposisiTerakhir = $this->db->select('min(pekerjaan_disposisi_waktu_start) AS awal, max(pekerjaan_disposisi_waktu_finish) AS akhir')->get_where('dec.dec_pekerjaan_disposisi', ['id_pekerjaan' => $value['id_pekerjaan'], 'pekerjaan_disposisi_status' => '5'])->row_array();

        $this->db->where('pekerjaan_id', $value['id_pekerjaan'])->update('dec.dec_pekerjaan', ['pekerjaan_estimasi_mulai' => $dataDisposisiTerakhir['awal'], 'pekerjaan_estimasi_selesai' => $dataDisposisiTerakhir['akhir']]);
      }
    }
  }

  public function fun_insert_cc($id_pekerjaan = null, $status = null, $id_user = null) {
    $user_implode = implode("','", $id_user);
    $cc_not_in = $this->db->get_where('dec.dec_pekerjaan_disposisi', ['id_pekerjaan' => $id_pekerjaan, 'pekerjaan_disposisi_status' => '8', 'is_cc' => $status, "id_user NOT IN ('".$user_implode."')" => null])->result_array();

    foreach ($cc_not_in as $value_not_in) {
      $data_cc = $this->db->get_where('global.global_pegawai', ['pegawai_nik' => $value_not_in['id_user']])->row_array();

      if ($status == 'y') dblog('I',  $id_pekerjaan, '' . $data_cc['pegawai_nama'] . ' Telah Dihapus Dari CC Non HPS', $isi['pegawai_nik']);
      else dblog('I',  $id_pekerjaan, '' . $data_cc['pegawai_nama'] . ' Telah Dihapus Dari CC HPS', $isi['pegawai_nik']);

      $this->db->where(['id_pekerjaan' => $id_pekerjaan, 'id_user' => $value_not_in['id_user'], 'pekerjaan_disposisi_status' => '8', 'is_cc' => $status])->delete('dec.dec_pekerjaan_disposisi');
    }

    foreach ($id_user as $value) {
      $ada_cc = $this->db->where(['id_pekerjaan' => $id_pekerjaan, 'pekerjaan_disposisi_status' => '8', 'id_user' => $value, 'is_cc' => $status])->count_all_results('dec.dec_pekerjaan_disposisi');
      if ($ada_cc == 0) {
        $data_disposisi_doc['pekerjaan_disposisi_id'] = create_id();
        $data_disposisi_doc['pekerjaan_disposisi_waktu'] = date('Y-m-d H:i:s');
        $data_disposisi_doc['id_user'] = anti_inject($value);
        $data_disposisi_doc['id_pekerjaan'] = $id_pekerjaan;
        $data_disposisi_doc['pekerjaan_disposisi_status'] = anti_inject('8');
        $data_disposisi_doc['id_penanggung_jawab'] = anti_inject('n');
        $data_disposisi_doc['is_cc'] = anti_inject($status);
        $data_disposisi_doc['is_aktif'] = 'y';
        $this->M_pekerjaan->insertPekerjaanDisposisi($data_disposisi_doc);
        ...
        /*notifikasi*/
        if ($status == 'y') dblog('I',  $id_pekerjaan, 'Pekerjaan Telah di CC ke ' . $data_cc['pegawai_nama'] . ' Sebagai CC Non HPS', $isi['pegawai_nik']);
        else dblog('I',  $id_pekerjaan, 'Pekerjaan Telah di CC ke ' . $data_cc['pegawai_nama'] . ' Sebagai CC HPS', $isi['pegawai_nik']);

        $tujuan = $data_cc['pegawai_nik'];
        $tujuan_nama = $data_cc['pegawai_nama'];
        $kalimat = "Pekerjaan telah di CC kepada anda";
        tasklog($id_pekerjaan, $data_disposisi_doc['pekerjaan_disposisi_status'], $data_cc['pegawai_nik'], $kalimat, 'n');
        sendWA($id_pekerjaan, $tujuan, $tujuan_nama, $kalimat);
        sendNotif($id_pekerjaan, $isi['pegawai_nik'], $tujuan, $kalimat);
        /*notifikasi*/
      }
    }
  }
