...
class Pekerjaan_langsung extends MX_Controller
{
    ...
    public function updatePekerjaanEdit()
    {
        ...
        if ($this->input->get_post('cc_hps')) {
            $this->db->query("DELETE FROM dec.dec_pekerjaan_disposisi WHERE is_cc = 'h' AND pekerjaan_disposisi_status = '8' AND id_pekerjaan='" . $pekerjaan_id . "'");
            foreach ($this->input->post('cc_hps') as $value) {
                $data_cc_hps = array(
                    'pekerjaan_disposisi_id' => create_id(),
                    'pekerjaan_disposisi_waktu' => date('Y-m-d H:i:s'),
                    'id_user' => $value,
                    'id_pekerjaan' => $this->input->post('pekerjaan_id_edit'),
                    'pekerjaan_disposisi_status' => '8',
                    'is_cc' => 'h',
                );
                $this->db->insert('dec.dec_pekerjaan_disposisi', $data_cc_hps);

                echo "<pre>";
                print_r($this->db->last_query());
                echo "</pre>";

                dblog('I', $data_cc_hps['id_pekerjaan'], 'Pekerjaan Telah DiCC');
            }
        }

        if ($this->input->get_post('cc_non_hps')) {
            $this->db->query("DELETE FROM dec.dec_pekerjaan_disposisi WHERE is_cc = 'y' AND pekerjaan_disposisi_status = '8' AND id_pekerjaan='" . $pekerjaan_id . "'");
            foreach ($this->input->post('cc_non_hps') as $value) {
                $data_cc = array(
                    'pekerjaan_disposisi_id' => create_id(),
                    'pekerjaan_disposisi_waktu' => date('Y-m-d H:i:s'),
                    'id_user' => $value,
                ...
                $this->db->insert('dec.dec_pekerjaan_disposisi', $data_cc);

                echo "<pre>";
                print_r($this->db->last_query());
                echo "</pre>";

                dblog('I', $data_cc['id_pekerjaan'], 'Pekerjaan Telah DiCC');
            }
        }
        if ($pekerjaan_id) {
            $data['pekerjaan_judul'] = $this->input->post('pekerjaan_judul_edit');
            $data['pekerjaan_waktu'] = $this->input->post('pekerjaan_waktu_edit');
    ...
    public function updatePekerjaanLangsungAksi()
    {
        ...
        $data['pekerjaan_waktu'] = $this->input->get_post('pekerjaan_waktu_aksi') . ' ' . date('H:i:s');
        // $data['pekerjaan_waktu_akhir'] = $this->input->get_post('pekerjaan_waktu_akhir_aksi');//
        ...
        if ($this->input->get_post('cc_id')) {
            $this->M_pekerjaan->deletePekerjaanDisposisi($pekerjaan_id, null, null, '8', 'y');
        ...
        if ($this->input->get_post('cc_hps_id')) {
            $this->M_pekerjaan->deletePekerjaanDisposisi($pekerjaan_id, null, null, '8', 'h');
