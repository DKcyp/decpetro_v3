    ...
    if ($this->input->get_post('id_user_send_vp')) {
      ...
      if ($this->input->get_post('id_user_send_vp_hps')) {
        ...
        foreach ($data_dokumen_send_hps as $val_dokumen) {
          $dokumen_ada = $this->db->query("SELECT * FROM dec.dec_pekerjaan_dokumen WHERE id_pekerjaan = '" . $this->input->get_post('pekerjaan_id') . "' AND id_create_awal = '" . $val_dokumen['id_create_awal'] . "' AND pekerjaan_dokumen_status ='3' AND pekerjaan_dokumen_nama = '" . $val_dokumen['pekerjaan_dokumen_nama'] . "' AND id_pekerjaan_template = '" . $val_dokumen['id_pekerjaan_template'] . "'  AND is_hps = 'y'")->row_array();
          $nomor_revisi = $this->db->query("SELECT max(pekerjaan_dokumen_revisi) as nomor_revisi FROM dec.dec_pekerjaan_dokumen WHERE pekerjaan_dokumen_id = '" . $val_dokumen['pekerjaan_dokumen_id'] . "'")->row_array();
          $nomor_revisi_baru = $nomor_revisi['nomor_revisi'];
          if (!empty($dokumen_ada) && ($dokumen_ada['pekerjaan_dokumen_nama'] == $val_dokumen['pekerjaan_dokumen_nama'] && $dokumen_ada['id_pekerjaan_template'] == $val_dokumen['id_pekerjaan_template'] && $dokumen_ada['id_create_awal'] == $val_dokumen['id_create_awal'])) {
            /*skip*/
      ...
      public function prosesSendVPIFC()
      {
        ...
        if ($this->input->get_post('id_user_cc')) {
          ...
          if ($this->input->get_post('id_user_cc_hps')) {
            ...
            $data_dokumen = $this->db->query("SELECT * FROM dec.dec_pekerjaan_dokumen WHERE id_pekerjaan = '" . $pekerjaan_id . "' AND is_lama = 'n' AND pekerjaan_dokumen_status >= '8' AND pekerjaan_dokumen_status <= '9' AND is_hps='n' AND id_create_awal IN (SELECT id_pegawai FROM global.global_bagian_detail WHERE id_bagian = '" . $data_bagian['id_bagian'] . "')  ")->result_array();
            foreach ($data_dokumen as $val_dokumen) {
              $sql_status = $this->db->query("SELECT * FROM dec.dec_pekerjaan_dokumen WHERE pekerjaan_dokumen_id = '" . $val_dokumen['pekerjaan_dokumen_id'] . "'");
              $data_status = $sql_status->row_array();
              $status_dokumen = '10';
            ...
            $data_dokumen_hps = $this->db->query("SELECT * FROM dec.dec_pekerjaan_dokumen WHERE id_pekerjaan = '" . $pekerjaan_id . "' AND is_lama = 'n' AND pekerjaan_dokumen_status >= '8' AND pekerjaan_dokumen_status <= '9' AND is_hps='y' AND id_create_awal IN (SELECT id_pegawai FROM global.global_bagian_detail WHERE id_bagian = '" . $data_bagian['id_bagian'] . "')  ")->result_array();
            foreach ($data_dokumen_hps as $val_dokumen) {
              $sql_status = $this->db->query("SELECT * FROM dec.dec_pekerjaan_dokumen WHERE pekerjaan_dokumen_id = '" . $val_dokumen['pekerjaan_dokumen_id'] . "'");
              $data_status = $sql_status->row_array();
              $status_dokumen = '10';
    ...
    if ($this->input->get_post('id_user_vp_avp')) {
      ...
      if ($pekerjaan_id) {
        if ($this->input->post('id_klasifikasi_pekerjaan_avp') && $this->input->get_post('pekerjaan_waktu_akhir_avp')) {
          ...
          $sql_nomor = $this->db->query("SELECT SPLIT_PART(pekerjaan_nomor,'-',1) as pekerjaan_nomornya FROM dec.dec_pekerjaan WHERE pekerjaan_nomor LIKE '%" . date('Y') . "%' AND pekerjaan_nomor IS NOT NULL " . $where . " ORDER BY CAST(SPLIT_PART(pekerjaan_nomor, '-', 1) as FLOAT) DESC");
        ...
        // $data['pekerjaan_nomor'] = ($isi_pekerjaan['pekerjaan_nomor'] == null) ? (sprintf("%03d", $nomor[0] + 1)) . '-' . $isi_klasifikasi['klasifikasi_pekerjaan_nama'] . '-' . date('Y') : $isi_pekerjaan['pekerjaan_nomor'];
          $data['pekerjaan_nomor'] = ($isi_pekerjaan['pekerjaan_nomor'] == null) ? $nomor_baru . '-' . $isi_klasifikasi['klasifikasi_pekerjaan_nama'] . '-' . date('Y') : $isi_pekerjaan['pekerjaan_nomor'];
        ...
        // $this->db->query("UPDATE dec.dec_pekerjaan_dokumen SET pekerjaan_dokumen_waktu = '" . date('Y-m-d H:i:s') . "' WHERE pekerjaan_dokumen_id = '" . $param['pekerjaan_dokumen_id'] . "' AND pekerjaan_dokumen_status >= '6'");
            ...
            if ($this->input->get_post('status') == '7') {
              ...
              foreach ($disposisi as $value) {
                ...
                foreach ($dataStaf as $valueStaf) :
                  $this->db->query("UPDATE dec.dec_pekerjaan_disposisi SET is_proses = null WHERE is_aktif = 'y' AND id_pekerjaan = '" . $this->input->get_post('pekerjaan_id') . "' AND id_user = '" . $valueStaf['id_user'] . "'");
                  /* Staf */
                  /* Notif DOF */
                  $dari = $isi['pegawai_nik'];
          ...
          public function prosesRejectBerjalanIFC()
          {
            ...
            if ($data_avp_reject) {
              ...
              $data_staf = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status = '11' AND is_aktif = 'y' AND id_pekerjaan = '" . $this->input->get_post('pekerjaan_id') . "'")->result_array();
              $data_avp = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status = '12' AND is_aktif = 'y' AND id_pekerjaan = '" . $this->input->get_post('pekerjaan_id') . "'")->result_array();

              /*      update status staf ke null*/
              foreach ($data_staf as $value) {
                $this->db->query("UPDATE dec.dec_pekerjaan_disposisi SET is_proses = NULL WHERE id_pekerjaan = '" . $this->input->get_post('pekerjaan_id') . "' AND is_aktif = 'y' AND id_user = '" . $value['id_user'] . "' AND pekerjaan_disposisi_status = '11' ");

                $data_users = $this->db->get_where('global.global_pegawai', array('pegawai_nik' => $value['id_user']))->row_array();
              ...
              foreach ($data_avp as $value) {
                /*hapus data avp agar tidak terdoble*/
                $this->db->query("DELETE FROM dec.dec_pekerjaan_disposisi WHERE id_pekerjaan = '" . $this->input->get_post('pekerjaan_id') . "' AND is_aktif = 'y' AND id_user = '" . $value['id_user'] . "' AND pekerjaan_disposisi_status = '12' ");
              }
    ...
    // }
      ...
      /*if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
          ...
          public function prosesApproveIFAAVP()
          {
            ...
            dblog('I',  $pekerjaan_id, 'Pekerjaan IFA Telah di Reviewed Oleh AVP Peminta Jasa', $isi['pegawai_nik']);
            /* Pekerjaan */
    ...
    // }
      ...
      /*if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
          ...
          public function prosesApproveIFAVP()
          {
            ...
            dblog('I',  $pekerjaan_id, 'Pekerjaan IFA Telah di Approved Oleh VP Peminta Jasa', $isi['pegawai_nik']);
            /* Pekerjaan */
      ...
      /*if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
          ...
          public function prosesRejectIFA()
          {
            ...
            foreach ($data_cek_pekerjaan as $key => $value) {
              ...
              $user_data = $this->db->get_where('global.global_pegawai', array('pegawai_nik' => $value['id_user']))->row_array();

              $dari = $isi['pegawai_nik'];
              $tujuan = $user_data['pegawai_nik'];
              $tujuan_nama = $user_data['pegawai_nama'];
          ...
          public function simpanAksi()
          {
            ...
            if ($this->input->get_post('pekerjaan_dokumen_status') == 'n') {
              $sql_customer = $this->db->query("SELECT * FROM dec.dec_pekerjaan_disposisi WHERE pekerjaan_disposisi_status IN('8','9','10') AND id_user = '" . $isi['pegawai_nik'] . "' AND id_pekerjaan = '" . $data_status['id_pekerjaan']
                . "'")->num_rows();
              if ($sql_customer > 0) {
                $nomor_revisi_baru = $nomor_revisi['nomor_revisi'] + 1;
              } else {
