...
<div class="page-content">
  ...
  $dataLimitPekerjaan = $this->db->query("SELECT * FROM global.global_limit_pekerjaan WHERE bagian_kode = '" . $session['pegawai_id_bag'] . "'")->row_array();
            ...
            <form id="filter">
              <div class="row">
                ...
                <div class="form-group col-md-2">
                  <label>&emsp;</label>
                  <button class="btn btn-primary form-control" type="button" name="cari_usulan" id="cari_usulan">Cari</button>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <ul class="nav nav-tabs">
              <input type="text" value="<?= $session['pegawai_id_dep'] ?>" id="id_dep" name="id_dep" style="display: none;">
              ...
              <li class="nav-item">
                <a class="nav-link active bg-secondary bg-gradient" href="#usulan" onclick="div_usulan()" id="link_div_usulan">
                  Usulan
          ...
          <div class="card-body" id="div_usulan">
            <div>
              <?php if ($dataLoadPekerjaan['count'] >= $dataLimitPekerjaan['limit_pekerjaan_total']) : ?>
                <button onclick="funwarning('<?= $dataLoadPekerjaan['count'] ?>');" data-toggle="modal" data-target="#modal_warning" type="button" class="btn btn-primary float-end">Tambah</button>
                <!-- <span class="alert alert-warning float-end">Masih ada <?= $dataLoadPekerjaan['count'] ?> Pekerjaan yang menunggu proses</span> -->
              <?php else : ?>
          ...
          <div class="card-body" id="div_ifi" style="display:none">
            <h4 class="card-title mb-4">Pekerjaan IFI</h4>
          ...
          <div class="card-body" id="div_ifa" style="display:none">
            <h4 class="card-title mb-4">Pekerjaan IFA</h4>
          ...
          <div class="card-body" id="div_ifc" style="display:none">
            <h4 class="card-title mb-4">Pekerjaan IFC</h4>
          ...
          <div class="card-body" id="div_ift" style="display:none">
            <h4 class="card-title mb-4">Pekerjaan IFT</h4>
          ...
          <div class="card-body" id="div_ifr" style="display:none">
            <h4 class="card-title mb-4">Pekerjaan IFR</h4>
        ...
        <div class="modal-body">
          <form id="form_modal_usulan" enctype="multipart/form-data">
            ...
            <div class="card-body row">
              <div class="form-group row col-md-12">
                ...
                <select name="pekerjaan_tahun" id="pekerjaan_tahun" class="form-control col-md-8" maxlength="4">
                  <?php for ($year = 2022; $year <= date('Y', strtotime('+1 year')); $year++) : ?>
            ...
            <div class="modal-footer justify-content-between">
              <button type="button" id="close" class="btn btn-default btn-outline-dark" data-dismiss="modal" onclick="fun_close_usulan()">Close</button>
  ...
  <div class="modal fade" id="modal_warning">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="modal_warning_title">Extend Dokumen IFC</h4>
        </div>
        <div class="modal-body">
          <div class="card-body row" id="formDiv">
            <table id="table_warning" class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th>No</th>
                  <th>No Pekerjaan</th>
  ...
  <script type="text/javascript">
      ...
      "columns": [{
          ...
          render: function(data, type, full, meta) {
            ...
            if (full.pekerjaan_nomor) {
              ...
              if (full.milik == 'y' && full.pekerjaan_nomor == null) {
                nomor_isi = '';
              } else if (full.milik == 'y' && <?= $session['pegawai_nik'] ?> != <?= $admin_sistemnya ?> && (full.perencana_proses == '0' || full.avp_proses == '0')) {
                nomor_isi = '<span class="badge" style="background-color:#c13333 ">' + nomor.join('-') + '</span>';
              } else if (full.milik == 'y' && <?= $session['pegawai_nik'] ?> != <?= $admin_sistemnya ?> && (full.perencana_proses == '1' || full.avp_proses == '1')) {
        ...
        },
        {
          "render": function(data, type, full, meta) {
            ...
            if ($('#id_dep').val() == 'E53000') {
              if (full.pekerjaan_status == 5 && full.perencana_proses == '1') {
                if (full.revisi_dokumen_berjalan_lama > 0) {
                  warna = '#CC6600';
                  status = 'IFA Rev ' + full.revisi_dokumen_berjalan_baru + ' - Send AVP ';
                } else {
                  warna = '#CC4200';
                  status = 'IFA - Send AVP'
                }
                ...
                warna = '#CC6600';
                if (full.revisi_dokumen > '0') {
                  status = 'IFA Rev ' + full.revisi_dokumen_lama + ' - In Progress'
                } else {
                  status = 'IFA - In Progress'
                }

              } else if (full.pekerjaan_status == 5 && full.status_avp == '0') {
                if (full.revisi_dokumen_berjalan > 0) {
                  warna = '#CC6600';
                  status = 'IFA Rev ' + full.revisi_dokumen_berjalan + ' - In Progress ';
                } else {
                  warna = '#CC6600';
                  status = 'In Progress';
                }
                ...
                //   warna = 'blue';
                //   font = 'white';
                //   status = 'IFA Revisi - untuk direview AVP Bidang ' + full.bagian_nama        
              } else if ((full.pekerjaan_status == 6 || full.status_avp == '1') && full.revisi_dokumen > '0' && full.avp_koor == '1' && full.avp_terkait_belum > 0) {
                warna = 'blue';
                font = 'white';
                status = 'IFA Revisi - untuk direview AVP Bidang ' + full.avp_terkait_bagian
              } else if ((full.pekerjaan_status == 6 || full.status_avp == '1') && full.avp_koor == '1' && full.avp_terkait_belum > 0) {
                warna = 'blue';
                font = 'white';
                // status = 'IFA - menunggu review AVP ' + full.avp_terkait_bagian
                if (full.dokumen_revisi_berjalan_rev > 0) {
                  status = 'IFA Rev ' + full.revisi_dokumen_berjalan_baru + '  - menunggu review AVP (';
                  var uniqueItems = [...new Set(full.avp_belum_proses)]; // Remove duplicates
                  status += uniqueItems.map(item => ` ${item}`).join(' , ');
                  status += ')';
                } else {
                ...
                font = 'white';
                if (full.dokumen_revisi_berjalan_rev > 0) {
                  if (full.dokumen_sudah_proses > 0) {
                    status = 'IFA Rev '+full.revisi_dokumen_berjalan_baru+' - untuk dikirim ke VP oleh AVP ' + full.bagian_nama +' ';
                  } else {
                    status = 'IFA Rev '+full.revisi_dokumen_berjalan_baru+' - untuk dikirim ke VP oleh AVP (';
                    var uniqueItems = [...new Set(full.avp_belum_proses)]; // Remove duplicates
                    status += uniqueItems.map(item => ` ${item}`).join(' , ');
                    status += ')';
                  }
                ...
                warna = 'red';
                font = 'white';
                status = 'IFA Revisi - Send VP'
              } else if (full.pekerjaan_status == 7) {
                warna = 'red';
                font = 'white';
                if (full.revisi_dokumen_berjalan_baru > 0) {
                  status = 'IFA Rev '+full.revisi_dokumen_berjalan_baru+' - Send VP'
                } else {
                  status = 'IFA - Send VP'
                }
              }
              ...
              if (full.revisi_dokumen_berjalan > 0) {
                warna = 'red';
                font = 'white';
                status = 'In Progress - Revisi'
              } else if (full.revisi_dokumen > 0) {
                warna = 'red';
                font = 'white';
                status = 'IFA - Revisi'
              } else {
        ...
        },
        {
          render: function(data, type, full, meta) {
            var text = '';
            if (full.perencana_proses_belum > 0) {
              text = '<span title="Belum Upload Dokumen/Inprogres" style="background-color:#ff6257">' + full.pekerjaan_isi_proses + '</span>';
            } else if (full.avp_proses_belum > 0) {
              text = '<span title="Proses Approve AVP" style="background-color:yellow">' + full.pekerjaan_isi_proses + '</span>';
            } else {
              text = '<span title="Proses Approve VP">' + full.pekerjaan_isi_proses + '</span>';
            }
            return text
          }
        },
        {
          render: function(data, type, full, meta) {
            var text = '';
            if (full.perencana_mesin_belum > 0) {
              text = '<span title="Belum Upload Dokumen/Inprogres" style="background-color:#ff6257" id="proses_text">' + full.pekerjaan_isi_mesin + '</span>';
            } else if (full.avp_mesin_belum > 0) {
              text = '<span title="Proses Approve AVP" style="background-color:yellow" id="proses_text">' + full.pekerjaan_isi_mesin + '</span>';
            } else {
              text = '<span title="Proses Approve VP" id="proses_text">' + full.pekerjaan_isi_mesin + '</span>';
            }
            return text
          }
        },
        {
          render: function(data, type, full, meta) {
            var text = '';
            if (full.perencana_listrik_belum > 0) {
              text = '<span title="Belum Upload Dokumen/Inprogres" style="background-color:#ff6257">' + full.pekerjaan_isi_listrik + '</span>';
            } else if (full.avp_listrik_belum > 0) {
              text = '<span title="Proses Approve AVP" style="background-color:yellow">' + full.pekerjaan_isi_listrik + '</span>';
            } else {
              text = '<span title="Proses Approve VP">' + full.pekerjaan_isi_listrik + '</span>';
            }
            return text
          }
        },
        {
          render: function(data, type, full, meta) {
            var text = '';
            if (full.perencana_instrumen_belum > 0) {
              text = '<span title="Belum Upload Dokumen/Inprogres" style="background-color:#ff6257">' + full.pekerjaan_isi_instrumen + '</span>';
            } else if (full.avp_instrumen_belum > 0) {
              text = '<span title="Proses Approve AVP" style="background-color:yellow">' + full.pekerjaan_isi_instrumen + '</span>';
            } else {
              text = '<span title="Proses Approve VP">' + full.pekerjaan_isi_instrumen + '</span>';
            }
            return text
          }
        },
        {
          render: function(data, type, full, meta) {
            var text = '';
            if (full.perencana_sipil_belum > 0) {
              text = '<span title="Belum Upload Dokumen/Inprogres" style="background-color:#ff6257">' + full.pekerjaan_isi_sipil + '</span>';
            } else if (full.avp_sipil_belum > 0) {
              text = '<span title="Proses Approve AVP" style="background-color:yellow">' + full.pekerjaan_isi_sipil + '</span>';
            } else {
              text = '<span title="Proses Approve VP">' + full.pekerjaan_isi_sipil + '</span>';
            }
            return text
          }
        },
        ...
        },
        {
          "render": function(data, type, full, meta) {
            ...
            if (full.pekerjaan_status == 0) {
              ...
              warna = '#FF8000';
              if (full.revisi_dokumen_baru > 0) {
                status = 'IFA Rev '+full.revisi_dokumen_baru+' - Send User'
              } else {
                status = 'IFA - Send User'
              }
            } else if (full.pekerjaan_status == 9) {
      ...
      "columns": [{
          render: function(data, type, full, meta) {
            ...
            if (full.pekerjaan_nomor) {
              ...
              if (full.milik == 'y' && full.pekerjaan_nomor == null) {
                nomor_isi = '';
              } else if (full.milik == 'y' && full.is_proses != 'y') {
                nomor_isi = '<span class="badge" style="background-color:#c13333 ">' + nomor.join('-') + '</span>';
              } else {
                nomor_isi = nomor.join('-');
              }
      ...
      "columns": [{
          render: function(data, type, full, meta) {
            ...
            if (full.pekerjaan_nomor) {
              ...
              if (full.milik == 'y' && full.pekerjaan_nomor == null) {
                nomor_isi = '';
              } else if (full.milik == 'y' && is_proses != 'y') {
                nomor_isi = '<span class="badge" style="background-color:#c13333 ">' + nomor.join('-') + '</span>';
              } else {
                nomor_isi = nomor.join('-');
              }
    ...
    /* FROM CARI SUBMIT */
    $('#cari_usulan').on('click', function(e) {
    ...
    /* Klik Tambah */
    function funwarning(count) {
      if ($.fn.DataTable.isDataTable('#table_warning')) {
        $('#table_warning').DataTable().destroy();
      }
      $('#table_warning-search').remove();
      $('#modal_warning_title').html("Masih ada " + count + " Pekerjaan yang menunggu proses");
      var table_warning = $('#table_warning').DataTable({
        bLengthChange: false,
        searching: false,
        ajax: {
          url: '<?= base_url() ?>/project/pekerjaan_usulan/pekerjaan_warning',
          dataSrc: ''
        },
        columns: [{
            render: function(data, type, full, meta) {
          ...
          {
            render: function(data, type, full, meta) {
              var date = new Date(full.pekerjaan_waktu);
      ...
      $('#table_warning thead').after('<tr id="table_warning-search"></tr>');
      $('#table_warning thead th').each(function() {
        $('#table_warning-search').append('<td><input type="text" class="form-control" placeholder="Search ' + $(this).text() + '" style="width: 100%;"></td>');
      });
      $('#table_warning-search input').on('keyup change', function() {
        var columnIndex = $(this).closest('td').index();
        table_warning.column(columnIndex).search(this.value).draw();
      });
    }
                      ...
                      for (var i = 0; i < files.length; i++) {
                        ...
                        if (extension !== 'pdf' && extension !== 'xls' && extension !== 'xlsx') {
                          Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Please select a PDF Or EXCEL  file.',
                          });
                          self.filebox('clear');
                          return;
                        } else {
