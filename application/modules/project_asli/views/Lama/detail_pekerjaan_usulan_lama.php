...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-12">
      <!-- <?php if ($_GET['rkap'] == 0): ?>
        <a href="javascript:;" onclick="call_ajax_page('project/Non_RKAP')" class="btn btn-success"><u><i class="fa fa-arrow-left"></i> Kembali</u></a>
      <?php else: ?>
        <a href="javascript:;" onclick="call_ajax_page('project/RKAP')" class="btn btn-success"><u><i class="fa fa-arrow-left"></i> Kembali</u></a>
        <?php endif ?> -->
        <a href="javascript:;" onclick="window.close();" class="btn btn-success"><u><i class="fa fa-arrow-left"></i> Tutup</u></a>
          ...
          <div class="card-body">
            ...
            <div class="row task-dates">
              ...
              <?php elseif ($_GET['aksi'] == 'ifa') : ?>
                ...
                <div class="col-sm-4 col-md-3" id="btn_upload_hps" style="display:none;margin-right:-50px">
                  <div class="mt-4">
                    <button type="button" class="btn btn-success col-10" onclick="fun_upload_hps('<?= $this->input->get('pekerjaan_id') ?>','<?= $_GET['status'] ?>')">Upload Dok Internal</button>
                  </div>
                </div>
                <div class="col-sm-4 col-md-3" id="btn_upload" style="display: none;margin-right:-50px">
                  <div class="mt-4">
              ...
              <div class="col-lg-12">
                <div class="card">
                  <div class="card-body">
                    ...
                    // CC
                      $total = 100;
                      if (isset($dataJumlah[0]['pekerjaan_disposisi_status']) == '3') {
                    ...
                    // CC
                      ...
                      <ul>
                        <?php if (isset($dataJumlah)) : ?>
                          <!-- CC -->
                          <?php if (isset($dataJumlah[0]['pekerjaan_disposisi_status']) == '3') : ?>
            ...
            <form id="form_modal_vp">
              ...
              <div class="card-body row">
                ...
                <div class="form-group row col-md-12" id="div_pekerjaan_disposisi_catatan">
                  ...
                  <textarea name="pekerjaan_disposisi_catatan" id="pekerjaan_disposisi_catatan" class="form-control col-md-8" placeholder="Catatan Disposisi"></textarea>
                </div>
              </div>
            ...
            <form id="form_modal_avp">
              ...
              <div class="card-body row">
                ...
                <div class="form-group row col-md-12" id="div_pekerjaan_waktu_akhir_avp" style="display:none">
                  ...
                  <input type="date" name="pekerjaan_waktu_akhir_avp" id="pekerjaan_waktu_akhir_avp" style="background-color: pink;" class="form-control col-md-8" value="<?= date('Y-m-d') ?>">
                </div>
  ...
  $(function() {
    ...
    $.getJSON('<?= base_url('project/pekerjaan_usulan/getPekerjaan') ?>', {
      ...
      if ('<?= $_GET['aksi'] ?>' == 'usulan') {
        ...
        } else if (json.pekerjaan_status == '8' && json.id_user == '<?= $data_session['pegawai_nik'] ?>') {
          /* Approve PIC */
          $('#btn_upload').css('display', 'none');
          $('#btn_upload_hps').css('display', 'none');
          $('#btn_progress').css('display', 'none');
...
$('#table_dokumen').DataTable({
  ...
  },
  {
    "render": function(data, type, full, meta) {
            // console.log(full);
      var aksi = '';
      if (full.id_bagian == $('#session_bagian').val() && full.avp == 'y' && full.is_proses != 'y' && full.vp != 'y' && full.pekerjaan_dokumen_status != 'x`0') {
...
$('#table_dokumen_ifa').DataTable({
  ...
  },
  {
    "render": function(data, type, full, meta) {
      var data = '';
      if (full.pekerjaan_dokumen_status == '0') {
        var data = 'IFA Rev. ' + parseInt(full.pekerjaan_dokumen_revisi);
      } else if (full.pekerjaan_dokumen_status == '1') {
        var data = 'Draft';
      } else if (full.pekerjaan_dokumen_status == '2') {
        var data = 'Menunggu Review AVP';
  ...
  setTimeout(function() {
    $('#dg_document').edatagrid({
      ...
      onBeginEdit: function(index,row){
          /* Combobox */
        var cb = $(this).datagrid('getEditor',{index:index,field:'pekerjaan_template_nama'});
        $(cb.target).combobox('setValue',row.id_pekerjaan_template);
          /* Combobox */

          /* File */
        var ed = $(this).datagrid('getEditor',{index:index,field:'pekerjaan_dokumen_file'});
...
function fun_approve_berjalan(id, text) {
  Swal.fire({
    ...
    if (result.value) {
      ...
      // if ($('#is_rkap').val() == '1') {
      //   call_ajax_page('project/RKAP#link_div_berjalan');
      // } else {
      //   call_ajax_page('project/Non_RKAP#link_div_berjalan');
      // }
...
$('#form_modal_send_vp').on('submit', function(e) {
  e.preventDefault();
  $.getJSON('<?= base_url('project/pekerjaan_usulan/getStatusKoor') ?>', {pekerjaan_id: $('#id_pekerjaan_send_vp').val()}, function(json) {
    if (json.total > 0) {
      Swal.fire(
        'Peringatan!',
        'Masih Ada Dokumen Yang Belum Siap!',
        'warning'
        );
    } else {
...
$('#form_modal_send_vp_ifc').on('submit', function(e) {
  ...
  $.ajax({
    ...
    data: {
      ...
      id_user_send_vp_hps: $('#id_user_send_vp_ifc_hps').val(),
    },
    type: 'POST',
    dataType: 'HTML',
      // processData: false,
...
function fun_lihat(file, id) {
  ...
  $.getJSON('<?= base_url('project/pekerjaan_usulan/lihatDokumen') ?>', {id: id}, function(json) {
...
function fun_draft_dokumen_ifc_hps(id) {
  ...
  $.ajax({
    ...
    data: $('#form_upload_dokumen_ifc_hps').serialize(),
    dataType: 'HTML',
    type: 'POST',
    success: function(result) {
